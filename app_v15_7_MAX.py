...
def init_session_state_v157() -> None:
    """
    Inicializa todas as chaves de sessão necessárias para o V15.7 MAX.
...
def construir_navegacao_v157() -> str:
    ...
    st.sidebar.markdown(
        ...
        7️⃣ Modo 6 Acertos V15.7 MAX<br/>
...
# ============================================================
# REGIME ULTRA — BARÔMETRO DA ESTRADA (V15.7 MAX)
# ============================================================
...
# ============================================================
# k* ULTRA REAL — SENTINELA DOS GUARDAS (V15.7 MAX)
# ============================================================
...
# ============================================================
# IDX ULTRA — CONTEXTO ESTRUTURAL GLOBAL (V15.7 MAX)
# ============================================================
...
# ============================================================
# Diagnóstico de Impacto & Mini Plano de Calibração (V15.7 MAX)
# ============================================================

def gerar_diagnostico_impacto_v157(
    idx_alvo: int,
...
def gerar_plano_calibracao_v157(
    regime_info: RegimeInfo,
    k_info: KStarInfo,
) -> PlanoCalibracao:
    """
...
def painel_pipeline_v14_flex_ultra_v157() -> None:
    """
    Painel estrutural do V14-FLEX ULTRA dentro do V15.7 MAX.
    ...
    with col_diag:
        flag_diag = st.checkbox(
            ...
            value=True,
            key="ck_diag_impacto_v157",
        )
    with col_plano:
        flag_plano = st.checkbox(
            "📋 Gerar mini plano de calibração",
            value=True,
            key="ck_plano_calibracao_v157",
        )

    n_series_total = len(df)

    if flag_diag:
        diag = gerar_diagnostico_impacto_v157(
            idx_alvo=int(idx_alvo),
    ...
    if flag_plano:
        plano = gerar_plano_calibracao_v157(regime_info, k_info)
    ...
    st.caption(
        "Este painel é a base estrutural do V15.7 MAX. "
...
# ------------------------------------------------------------
# ============================================================
# PARTE 3/6 — Replay LIGHT / ULTRA / Unitário (V15.7 MAX)
# ============================================================
...
def obter_janela_para_replay(df: pd.DataFrame, idx_alvo: int, largura: int) -> pd.DataFrame:
    """
    Retorna a janela:
        DF[(idx >= idx_alvo - largura) & (idx <= idx_alvo - 1)]
    Compatível com FLEX ULTRA e com idx criado automaticamente.
    """
    if "idx" not in df.columns:
        df["idx"] = np.arange(1, len(df) + 1, dtype=int)
...
def ponderar_listas(l1: List[int], l2: List[int], peso1: float = 0.5, peso2: float = 0.5) -> List[int]:
    """Média ponderada simples com arredondamento."""
    if len(l1) != len(l2):
        return l1
    return [int(round(a * peso1 + b * peso2)) for a, b in zip(l1, l2)]
...
def replay_light_v157(df: pd.DataFrame, idx_alvo: int, largura: int = 30) -> Dict[str, Any]:
    """
    Replay LIGHT — V15.7 MAX:
    - Janela curta (robustez local)
    - Similaridade simples
    - Previsão direta, rápida, leve
    """
    if "idx" not in df.columns:
    ...
    if not serie_prev or not serie_real:
        return {
            "serie_prevista": [],
            "serie_real": [],
            "descricao": "Não foi possível extrair séries anterior/atual."
        }
    ...
    if not melhores:
        ...
        if len(top3) == 1:
            ...
            media_array = np.mean([p for _, p in top3], axis=0)
            prev = [int(round(x)) for x in media_array]

    return {
        "serie_prevista": prev,
        "serie_real": serie_real,
        "descricao": f"Replay LIGHT (V15.7 MAX) — janela {largura}",
    }
...
def replay_ultra_v157(df: pd.DataFrame, idx_alvo: int, largura: int = 60) -> Dict[str, Any]:
    """
    Replay ULTRA — V15.7 MAX:
    - Janela larga
    - Similaridade S6-like leve
    - Combinação multi-pontos (mais forte que LIGHT)
    """
    if "idx" not in df.columns:
        df["idx"] = np.arange(1, len(df) + 1)
    ...
    if not serie_prev:
        return {
            "serie_prevista": [],
            "serie_real": [],
            "descricao": "Falha ao extrair série anterior."
        }
    ...
    if not melhores:
        ...
        acc = np.zeros(n_pass)
        for p, (_, linha) in zip(pesos, top5):
            acc += p * np.array(linha)

        prev = [int(round(x)) for x in acc]

    return {
        "serie_prevista": prev,
        "serie_real": serie_real,
        "descricao": f"Replay ULTRA (V15.7 MAX) — janela {largura}",
    }
...
def replay_unitario_v157(df: pd.DataFrame, idx_alvo: int) -> Dict[str, Any]:
    """
    Replay ULTRA Unitário — V15.7 MAX:
    - Operação focada somente na série alvo
    - Micro-deslocamento + ruído mínimo
    """
    if "idx" not in df.columns:
        df["idx"] = np.arange(1, len(df) + 1)
    ...
    prev = [int(round(a * 0.6 + b * 0.4)) for a, b in zip(serie_prev, deslocado)]
...
# ============================================================
# PAINEL — 💡 Replay LIGHT (V15.7 MAX)
# ============================================================

def painel_replay_light_v157() -> None:
    ...
    if st.button("Rodar Replay LIGHT", type="primary"):
        ...
        if st.session_state.get("flag_modo_seguro", True):
            if not limitar_operacao(len(df), limite_max, "Replay LIGHT V15.7"):
                return

        r = replay_light_v157(df, idx_alvo=idx_alvo, largura=largura)
        st.session_state["replay_light_result_v157"] = r

    r = st.session_state.get("replay_light_result_v157")
    if r:
...
# ============================================================
# PAINEL — 📅 Replay ULTRA (V15.7 MAX)
# ============================================================

def painel_replay_ultra_v157() -> None:
    ...
    if st.button("Rodar Replay ULTRA", type="primary"):
        ...
        if st.session_state.get("flag_modo_seguro", True):
            if not limitar_operacao(len(df), limite_max, "Replay ULTRA V15.7"):
                return

        r = replay_ultra_v157(df, idx_alvo=idx_alvo, largura=largura)
        st.session_state["replay_ultra_result_v157"] = r

    r = st.session_state.get("replay_ultra_result_v157")
    if r:
        st.markmarkdown("### Resultado Replay ULTRA")
        c1, c2 = st.columns(2)
        with c1:
            st.code(formatar_lista_passageiros(r["serie_prevista"]), language="text")
        with c2:
...
# ============================================================
# PAINEL — 🎯 Replay ULTRA Unitário (V15.7 MAX)
# ============================================================

def painel_replay_unitario_v157() -> None:
    ...
    if st.button("Rodar Replay ULTRA Unitário", type="primary"):
        ...
        if st.session_state.get("flag_modo_seguro", True):
            if not limitar_operacao(len(df), limite_max, "Replay ULTRA Unitário V15.7"):
                return

        r = replay_unitario_v157(df, idx_alvo=idx_alvo)
        st.session_state["replay_unitario_result_v157"] = r

    r = st.session_state.get("replay_unitario_result_v157")
    if r:
...
# ------------------------------------------------------------
# ============================================================
# PARTE 4/6 — Monitor de Risco (k/k*) + Ruído Condicional (V15.7 MAX)
# ============================================================

# ------------------------------------------------------------
# MONITOR DE RISCO — Cálculos de k e k*
# ------------------------------------------------------------

def montar_tabela_monitor_risco_v157() -> Optional[pd.DataFrame]:
    """
    Monta tabela consolidada com:
    - idx
    - k
    - k* (%)
    - regime k* (estável / atenção / crítico)

    Atualizado para V15.7 MAX com comportamento idêntico ao V15.6 MAX,
    mas reforçado para maior consistência interna e retrocompatibilidade.
    """
    df = st.session_state.get("df_historico")
    ...
    base["estado_kstar"] = base["k_star_pct"].apply(classificar)
    return base


# ------------------------------------------------------------
# RUÍDO ESTRUTURAL (NR% Global)
# ------------------------------------------------------------

def calcular_mapa_ruido_global_v157(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Mapa estrutural de ruído global (NR%):
    - médias
    - desvios
    - NR% por passageiro
    Com texto interpretativo atualizado para V15.7 MAX.
    """
    cols_pass = extrair_passageiros(df)
    if not cols_pass:
        return {}
    ...
    nr_med = float(np.mean(list(nr_pct.values)))
    nr_max = float(np.max(list(nr_pct.values)))

    if nr_med < 25:
        resumo = (
            "🟢 Ruído estrutural baixo — dispersão estável e compatível "
            "com ambientes premium para S6 profundo."
        )
    elif nr_med < 45:
        resumo = (
            "🟡 Ruído moderado — alguns passageiros ruidosos, mas ainda "
            "adequado para TURBO++ ULTRA com cautela moderada."
        )
    else:
        resumo = (
            "🔴 Ruído elevado — dispersão forte; recomenda reforçar coberturas "
            "e verificar regime k* antes de decisões agressivas."
        )
...
# ------------------------------------------------------------
# RUÍDO CONDICIONAL (NR% por regime k*)
# ------------------------------------------------------------

def calcular_mapa_ruido_condicional_v157(df: pd.DataFrame, tabela_risco: pd.DataFrame) -> Dict[str, Any]:
    """
    Divide o ruído por regime k* (estável / atenção / crítico).
    Mantém fidelidade ao comportamento V15.6 MAX.
    """
    cols_pass = extrair_passageiros(df)
    if not cols_pass:
        return {}
    ...
    for estado in ["estavel", "atencao", "critico"]:
        ...
        sp = sub[cols_pass].astype(float)
        medias = sp.mean()
        desvios = sp.std().replace(0, 1.0)
        nr_pct = (desvios / (medias.abs() + 1.0)) * 100.0
        ...
        resultados[estado] = {
            ...
            "nr_medio": float(np.mean(list(nr_pct.values))),
        }
...
def painel_monitor_risco_v157() -> None:
    ...
    df = st.session_state.get("df_historico")
    if df is None or df.empty:
        st.warning("Carregue o histórico antes de continuar.")
        return

    tabela = montar_tabela_monitor_risco_v157()
    if tabela is None or tabela.empty:
        st.info("Rode o Pipeline V14-FLEX ULTRA (V15.7 MAX) para gerar k/k*.")
        return

    st.dataframe(tabela.tail(50), use_container_width=True)
    ...
    with col1:
        st.metric("k médio", round(k_med, 4))
        st.metric("k máximo", int(k_max))
    with col2:
        st.metric("k* médio (%)", round(kstar_med, 1))
        st.metric("k* máximo (%)", round(kstar_max, 1))

    dist = tabela["estado_kstar"].value_counts(normalize=True) * 100
    st.markdown("### Distribuição de regimes k* (%)")
    st.json({k: round(v, 1) for k, v in dist.to_dict().items()})

    st.caption(
        "O monitor de risco identifica estabilidade, atenção e ruptura "
        "com base nos sentinelas k/k* atualizados."
    )


# ============================================================
# PAINEL — 📊 Ruído Condicional (V15.7 MAX)
# ============================================================

def painel_ruido_condicional_v157() -> None:
    st.markdown("## 📊 Ruído Condicional (NR%) — V15.7 MAX")
    ...
    tabela_risco = montar_tabela_monitor_risco_v157()
    if tabela_risco is None or tabela_risco.empty:
        st.info("Rode o Pipeline V14-FLEX ULTRA (V15.7 MAX) antes.")
        return

    st.markdown("### 🌐 Mapa Global de Ruído")
    ...
    mapa_cond = calcular_mapa_ruido_condicional_v157(df, tabela_risco)
    ...
    st.caption(
        "O ruído condicional é essencial para identificar trechos premium "
        "adequados para exploração TURBO++ ULTRA e Modo 6 Acertos."
    )
...
# ------------------------------------------------------------
# ============================================================
# PARTE 5/6 — Testes de Confiabilidade REAL (V15.7 MAX)
# ============================================================
...
def rodar_backtest_turbo_ultra_v157(
    ...
    """
    Backtest REAL usando o motor TURBO++ ULTRA V15.7 MAX:

    - Seleciona os últimos n_prev índices possíveis
    - Para cada índice i:
        * roda turbo_ultra_v157 para prever Ci
        * compara com a série real Ci
    - Calcula:
        * acertos totais (posição a posição)
        * flag teve_acerto (≥1 acerto)
    """
    if "idx" not in df.columns:
        df["idx"] = np.arange(1, len(df) + 1, dtype=int)
    ...
    # Precisamos de índices >= 2 para poder prever (usa série anterior)
    candidatos = [i for i in range(min_idx + 1, max_idx + 1)]
    if not candidatos:
        return BacktestResult(
            tabela=pd.DataFrame(),
            descricao="Sem índices suficientes para executar backtest REAL.",
        )
    ...
    for idx in candidatos:
        # Motor oficial V15.7 MAX (definido na PARTE 6/6)
        ...
        serie_real = [int(real_row.iloc[0][c]) for c in cols_pass]
        serie_prev = res["final"]

        ac_total = calcular_acerto_total(serie_real, serie_prev)
    ...
    if not resultados:
        return BacktestResult(
            tabela=pd.DataFrame(),
            descricao="Backtest não produziu resultados válidos.",
        )

    df_res = pd.DataFrame(resultados)
    return BacktestResult(
        tabela=df_res,
        descricao="Backtest REAL utilizando motor TURBO++ ULTRA (V15.7 MAX).",
    )


def sintetizar_confiabilidade_v157(df_res: pd.DataFrame) -> Dict[str, Any]:
    """
    Consolida a tabela de backtest em indicadores de confiabilidade:

    - p_ao_menos_um  → probabilidade de ter pelo menos 1 acerto
    - media_acertos  → média de acertos totais
    - n_prev         → quantidade de previsões testadas
    """
...
def gerar_texto_confiabilidade_v157(stats: Dict[str, Any], alvo: float) -> str:
    """
    Gera a leitura qualitativa da confiabilidade REAL,
    comparando com o alvo configurado.
    """
    ...
    if n_prev == 0:
        return (
            "Não foi possível calcular confiabilidade REAL, pois o backtest "
            "não produziu previsões válidas suficientes."
        )
    ...
    if p >= alvo:
        return (
            f"🟢 Confiabilidade em regime saudável: {pct}% dos casos com pelo menos 1 acerto, "
            f"acima ou igual ao alvo de {alvo_pct}%, considerando {n_prev} previsões testadas."
        )
    elif p >= alvo * 0.8:
        return (
            f"🟡 Confiabilidade intermediária: {pct}% dos casos com pelo menos 1 acerto, "
            f"próxima do alvo de {alvo_pct}%. Em trechos estruturalmente estáveis e com "
            "NR% moderado, o Modo 6 Acertos pode ser considerado de forma cirúrgica, "
            "sempre em conjunto com os diagnósticos de regime e ruído condicional."
        )
    else:
        return (
            f"🔴 Confiabilidade abaixo do desejado: {pct}% dos casos com pelo menos 1 acerto, "
            f"abaixo do alvo de {alvo_pct}%. É indicado reforçar calibração, revisar pesos "
            "do TURBO++ ULTRA, checar o barômetro de regime e o mapa de ruído condicional "
            "antes de decisões agressivas."
        )
...
def painel_testes_confiabilidade_v157() -> None:
    ...
    n_prev = st.number_input(
        ...
        value=int(st.session_state.get("confiabilidade_n_prev", 50)),
        step=10,
    ...
    alvo = st.slider(
        "Alvo de confiabilidade (P(≥1 acerto)):",
        min_value=0.40,
        max_value=0.90,
    ...
    if st.button("Rodar Testes de Confiabilidade REAL (TURBO++ ULTRA V15.7)", type="primary"):
        # Proteção Anti-Zumbi
        if st.session_state.get("flag_modo_seguro", True):
    ...
    back: BacktestResult = st.session_state.get("confiabilidade_resultados")
    if back is None or back.tabela is None or back.tabela.empty:
        st.info("Nenhum resultado de backtest disponível ainda. Rode o teste acima.")
        return

    st.markdown("### Tabela de Backtest — TURBO++ ULTRA (V15.7 MAX)")
    ...
    st.caption(
        "Os Testes de Confiabilidade REAL V15.7 MAX são o eixo central para decidir "
        "se o ambiente está pronto para ativar o Modo 6 Acertos, e também para "
        "alimentar o Relatório Final MAX com leituras quantitativas e qualitativas."
    )
...
# ------------------------------------------------------------
# ============================================================
# PARTE 6/6 — TURBO++ ULTRA V15.7 + Modo 6 Acertos + Relatório Final + main()
# ============================================================
...
def s6_profundo_v157(df: pd.DataFrame, idx_alvo: int) -> List[int]:
    """
    Núcleo determinístico (S6 Profundo) — V15.7 MAX:
...
def micro_leque_profundo_v157(serie_base: List[int], intensidade: float = 0.25) -> List[List[int]]:
    ...
    Intensidade controla amplitude dos deslocamentos.
    """
    leques = []
...
# ------------------------------------------------------------
# MONTE CARLO PROFUNDO (V15.7 MAX)
# ------------------------------------------------------------
...
def monte_carlo_profundo_v157(
    ...
    """
    Monte Carlo Profundo — gera amostras ruidosas em torno da série base.
    """
    out = []
...
# ------------------------------------------------------------
# DIVERGÊNCIA ENTRE S6 E MONTE CARLO (V15.7 MAX)
# ------------------------------------------------------------

def divergencia_s6_mc_v157(serie_s6: List[int], mc_list: List[List[int]]) -> float:
    """
    Mede o quanto o Monte Carlo se afasta do núcleo S6 (V15.7).
    Retorna valor médio de dispersão normalizado.
    """
...
# ------------------------------------------------------------
# PESOS DINÂMICOS CONTROLADOS POR k* (V15.7 MAX)
# ------------------------------------------------------------

def ajustar_pesos_por_kstar_v157(kstar_info: KStarInfo) -> Dict[str, float]:
    """
    Ajuste dinâmico de pesos S6 / MC / Micro-Leque conforme k* (V15.7):
    - k* baixo → S6 domina
    - k* médio → equilíbrio
    - k* alto → MC domina (turbulência)
    """
...
# ------------------------------------------------------------
# FUNÇÃO PRINCIPAL — TURBO++ ULTRA (V15.7 MAX)
# ------------------------------------------------------------
...
def turbo_ultra_v157(
    ...
    Combina:
    - S6 PROFUNDO V15.7 (núcleo determinístico)
    - Micro-Leque Profundo V15.7
    - Monte Carlo Profundo V15.7
    - Pesos guiados por k* local
    ...
    if idx_alvo <= 1:
        return {"erro": "Índice alvo inválido para TURBO++ ULTRA (precisa ser ≥ 2)."}
    ...
    if not serie_s6:
        return {"erro": "Não foi possível gerar núcleo S6 Profundo V15.7."}

    # --------------------------------------------------------
    # 2) Micro-Leque Profundo
    # --------------------------------------------------------
    leques = micro_leque_profundo_v157(serie_s6, intensidade=elasticidade_nucleo or 0.25)
    leques_flat = leques.copy()

    # --------------------------------------------------------
    # 3) Monte Carlo Profundo
    # --------------------------------------------------------
    mc_list = monte_carlo_profundo_v157(
        serie_s6,
        n_amostras=200,
        ruido_base=max(0.10, 0.65 * (1.0 + intensidade_turbulencia)),
    )

    div_s6_mc = divergencia_s6_mc_v157(serie_s6, mc_list)

    # --------------------------------------------------------
    # 4) Pesos guiados por k* local
    # --------------------------------------------------------
    ...
    pesos_dyn = ajustar_pesos_por_kstar_v157(k_info_local)

    # --------------------------------------------------------
    # 5) Interseção estatística (núcleo + coberturas)
    # --------------------------------------------------------
    n_pass = len(serie_s6)
    combinacoes = []
    ...
    arr = np.zeros(n_pass, dtype=float)
    for c in combinacoes:
        tmp = np.array(c, dtype=float)
        arr += (
            pesos_dyn["peso_s6"] * tmp
            + pesos_dyn["peso_micro"] * tmp
            + pesos_dyn["peso_mc"] * tmp
        )

    arr = arr / max(len(combinacoes), 1)
    final = [int(round(x)) for x in arr]

    # --------------------------------------------------------
    # REGISTRO EM SESSÃO (COMPATÍVEL COM V15.6)
    # --------------------------------------------------------
    resultado_sessao = {
        "serie_s6": serie_s6,
    ...
    st.session_state["turbo_ultra_logs"] = [
        f"S6: {serie_s6}",
        f"Pesos dinâmicos (k*): {pesos_dyn}",
        f"Divergência S6-MC: {div_s6_mc}",
        f"Final: {final}",
    ]

    # --------------------------------------------------------
    # PAINEL ANALÍTICO COMPLETO (ESTRUTURA RICA)
    # --------------------------------------------------------
    loc = locals()
    ...
    resultados = {
        ...
        "micro_leque_info": {
            "numeros": leques_flat[0] if leques_flat else serie_s6,
        ...
        "monte_carlo": {
            "numeros": mc_list[0] if mc_list else final,
        ...
        "k_star": {
            "valor": k_info_local.k_star_pct,
            "regime": k_info_local.estado,
            "forca": loc.get("k_star_forca"),
            "ajuste_pesos": {
                "peso_s6_final": pesos_dyn.get("peso_s6"),
                "peso_mc_final": pesos_dyn.get("peso_mc"),
                "peso_micro_final": pesos_dyn.get("peso_micro"),
            },
        },
        "idx_ultra": {
        ...
        "painel_debug": {
            ...
            "s6_raw": serie_s6,
            "mc_raw": mc_list[:20],
            "micro_raw": leques_flat,
            "k_series": st.session_state.get("serie_k"),
            "k_star_series": st.session_state.get("serie_k_star"),
    ...
    resultados["descricao"] = "Previsão TURBO++ ULTRA V15.7 MAX"

    # Atualiza sessão com versão rica também
    st.session_state["turbo_ultra_result"] = resultados

    return resultados


# ============================================================
# PAINEL — 🚀 TURBO++ ULTRA (V15.7 MAX)
# ============================================================
...
def painel_turbo_ultra_v157() -> None:
    ...
    idx_alvo = st.number_input(
        "Índice alvo (previsão TURBO++ ULTRA V15.7):",
        min_value=min_idx + 1,
        max_value=max_idx,
        value=max_idx,
        step=1,
    ...
    with st.expander("🔬 Ajustes Avançados do Motor — TURBO++ ULTRA V15.7"):
        ...
        suavizacao_idx = st.slider(
            ...
            min_value=0.00,
            max_value=1.00,
            value=float(av.get("suavizacao_idx", 0.25)),
            step=0.05,
        )
        ...
        profundidade_micro = st.slider(
            ...
            min_value=3,
            max_value=40,
            value=int(av.get("profundidade_micro", 15)),
            step=1,
        )
        ...
        fator_antirruido = st.slider(
            ...
            min_value=0.00,
            max_value=1.00,
            value=float(av.get("fator_antirruido", 0.40)),
            step=0.05,
        )
        ...
        elasticidade_nucleo = st.slider(
            ...
            min_value=0.00,
            max_value=1.00,
            value=float(av.get("elasticidade_nucleo", 0.20)),
            step=0.05,
        )
        ...
        intensidade_turbulencia = st.slider(
            ...
            min_value=0.00,
            max_value=1.00,
            value=float(av.get("intensidade_turbulencia", 0.30)),
            step=0.05,
        )
    ...
    if st.button("Rodar TURBO++ ULTRA V15.7", type="primary"):
        limite_max = st.session_state.get("limite_max_janela", 600)
        if st.session_state.get("flag_modo_seguro", True):
            if not limitar_operacao(len(df), limite_max, "TURBO++ ULTRA V15.7"):
                return
    ...
    if r:
        st.markdown("### 🔥 PREVISÃO FINAL TURBO++ ULTRA V15.7")
        ...
        with col2:
            st.subheader("Micro-Leque (amostras)")
        ...
        st.markdown("### ⚖️ Pesos e Parâmetros Utilizados (V15.7 MAX)")
        pesos_dict = r.get("pesos", {})
        av = st.session_state.get("turbo_ultra_avancado", {})

        st.json(
            {
                "peso_s6": pesos_dict.get("peso_s6", 0.5),
                "peso_mc": pesos_dict.get("peso_mc", 0.35),
                "peso_micro": pesos_dict.get("peso_micro", 0.15),
                "suavizacao_idx": pesos_dict.get("suavizacao_idx", av.get("suavizacao_idx", 0.25)),
                "profundidade_micro": pesos_dict.get("profundidade_micro", av.get("profundidade_micro", 15)),
                "fator_antirruido": pesos_dict.get("fator_antirruido", av.get("fator_antirruido", 0.40)),
                "elasticidade_nucleo": pesos_dict.get("elasticidade_nucleo", av.get("elasticidade_nucleo", 0.20)),
                "intensidade_turbulencia": pesos_dict.get("intensidade_turbulencia", av.get("intensidade_turbulencia", 0.30)),
            }
        )
...
def avaliar_prontidao_modo6_v157() -> Tuple[bool, str]:
    """
    Leitura integrada para Modo 6 Acertos V15.7:

    - Diagnóstico de impacto (regime + k*)
    - Confiabilidade REAL V15.7
    - Ruído Condicional
    - k* local (última série da estrada)
    """
    ...
    if diag is None or conf is None or conf.tabela is None or conf.tabela.empty:
        return False, (
            ...
            "Rode o Pipeline V14-FLEX ULTRA (V15.7) e os Testes de Confiabilidade REAL."
        )
    ...
    if mapa_cond and "estavel" in mapa_cond:
        vals = list(mapa_cond["estavel"]["nr_pct"].values())
        if vals:
            nr_estavel = float(np.mean(vals))
    ...
    if motivos:
        texto = (
            ...
            + "\n\nSugestão: fortalecer calibração, revisar pesos do TURBO++ ULTRA V15.7, "
    ...
    texto = (
        ...
        "Recomendação: utilizar Modo 6 Acertos V15.7 de forma cirúrgica, sempre "
        "associando o núcleo TURBO++ ULTRA, o mapa de ruído condicional e o plano "
        "de calibração."
    )
...
def painel_modo_6_acertos_v157() -> None:
    ...
    if df is None or df.empty:
        st.warning(
            "Carregue o histórico e rode o Pipeline V14-FLEX ULTRA (V15.7) + "
    ...
    exibir_bloco_mensagem(
        "Prontidão para Modo 6 Acertos (V15.7)",
    ...
    st.write(
        ...
        f"tomando a estrada de C1 até C{max_idx} como base."
    )

    turbo = st.session_state.get("turbo_ultra_result")
    if turbo is None or not turbo.get("final"):
        st.info(
            "Ainda não há uma previsão TURBO++ ULTRA V15.7 registrada. "
            "Vá até `🚀 Modo TURBO++ ULTRA ANTI-RUÍDO (V15.7 MAX)`, "
            "rode a previsão para o índice alvo desejado e depois volte aqui."
        )
        return

    serie_nucleo = turbo["final"]
    st.markdown("### 🔧 Núcleo proposto pelo TURBO++ ULTRA V15.7")
    st.code(formatar_lista_passageiros(serie_nucleo), language="text")

    st.markdown("### 🛰️ Coberturas derivadas do núcleo (Micro-Leque Profundo V15.7)")
    leques = micro_leque_profundo_v157(serie_nucleo, intensidade=0.25)
    ...
    st.write(
        "O Modo 6 Acertos V15.7 MAX permanece **semi-automático**: "
        "o sistema apresenta núcleo + coberturas + diagnóstico de prontidão, "
        "mas a decisão final é sempre do operador."
    )
    ...
    exibir_bloco_mensagem(
        "Resumo da seleção Modo 6 Acertos (V15.7)",
        texto_em_blocos(
            "Núcleo selecionado: "
    ...
    st.caption(
        "Importante: o Modo 6 Acertos V15.7 MAX não executa nenhuma ação automática "
        "no mundo real. Ele apenas organiza a informação, destacando ambientes "
        "mais ou menos favoráveis, com decisão final sempre do operador."
    )


# ------------------------------------------------------------
# PAINEL — 📑 Relatório Final V15.7 MAX
# ------------------------------------------------------------
...
def montar_relatorio_final_v157() -> str:
    ...
    Consolida em texto:
    - Histórico FLEX ULTRA
    - Diagnóstico de impacto (V15.7)
    - Sentinelas k/k*
    - Ruído global / condicional
    - Confiabilidade REAL V15.7
    - Núcleo TURBO++ ULTRA V15.7
    - Modo 6 Acertos V15.7
    """
    df = st.session_state.get("df_historico")
    diag: DiagnosticoImpacto = st.session_state.get("diag_impacto")
    ...
    if df is not None and not df.empty:
        ...
        linhas.append("=== HISTÓRICO FLEX ULTRA (V15.7 MAX) ===")
    ...
    # Diagnóstico de impacto
    linhas.append("=== DIAGNÓSTICO DE IMPACTO / REGIME (V15.7) ===")
    if diag is not None:
    ...
    # k / k* global
    linhas.append("=== SENTINELAS k / k* (GLOBAL V15.7) ===")
    if tabela_risco is not None and not tabela_risco.empty:
        ...
        linhas.append("Séries k/k* ainda não avaliadas (V15.7).")
        linhas.append("")

    # Ruído
    linhas.append("=== RUÍDO ESTRUTURAL / CONDICIONAL (NR%) (V15.7) ===")
    if mapa_global is not None and mapa_global.get("nr_pct"):
        ...
        linhas.append("Mapa de ruído global ainda não foi calculado (V15.7).")
        linhas.append("")
    ...
    # Plano de calibração
    linhas.append("=== MINI PLANO DE CALIBRAÇÃO (V15.7 MAX) ===")
    ...
    # Confiabilidade REAL
    linhas.append("=== TESTES DE CONFIABILIDADE REAL (V15.7) ===")
    if conf is not None and conf.tabela is not None and not conf.tabela.empty:
        ...
        linhas.append("")
        linhas.append("Leitura qualitativa da confiabilidade (V15.7):")
        linhas.append(
            texto_em_blocos(gerar_texto_confiabilidade_v157(stats, alvo), largura=90)
        )
        linhas.append("")
    else:
        linhas.append("Ainda não foram rodados Testes de Confiabilidade REAL (V15.7).")
        linhas.append("")
    ...
    if turbo is not None and turbo.get("final"):
        ...
        linhas.append("")
        linhas.append("Núcleo (S6 Profundo V15.7):")
        ...
        linhas.append("Nenhuma previsão TURBO++ ULTRA V15.7 registrada neste relatório.")
        linhas.append("")

    # Modo 6 Acertos
    linhas.append("=== MODO 6 ACERTOS — RESUMO (V15.7 MAX) ===")
    if modo6 is not None:
        ...
        linhas.append("Modo 6 Acertos ainda não foi utilizado nesta sessão (V15.7).")
        linhas.append("")
...
def painel_relatorio_final_v157() -> None:
    ...
    st.caption(
        "Este relatório consolida a visão do V15.7 MAX: histórico, regime, ruído, "
        "confiabilidade, núcleo TURBO++ ULTRA V15.7 e status do Modo 6 Acertos."
    )
