...
st.set_page_config(page_title="PredictCars V15.7 MAX — v16h57FP — BUILD AUDITÁVEL (micro encaixe de conversão + família estável)", page_icon="🚗", layout="wide")
...
# ============================================================
def v16h57FP_clear_mode6_packet_state():
    removed = []
    keys = [
        "modo6_listas",
        "modo6_listas_top10",
    ...
    try:
        ...
        pass
    st.session_state["v16h57FP_fresh_packet_removed_keys"] = removed
    st.session_state["v16h57FP_fresh_packet_ts"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    return removed
...
if painel == "🎯 Compressão do Alvo (Observacional)":
    ...
    # V16h57CN — SESSION STATE CONTROL (FORCE FRESH PACKET)
    # ------------------------------------------------------------
    _removed_fresh_keys = v16h57FP_clear_mode6_packet_state()
...
if painel == "🎯 Modo 6 Acertos — Execução":
    ...
    st.session_state["v16h57FP_exec_trace"] = []
...
try:
    ...
    if listas_ref:
        ...
        _trace_exec = st.session_state.get("v16h57FP_exec_trace", [])
        ...
        _src_trace = st.session_state.get("v16h57FP_source_detector", [])
