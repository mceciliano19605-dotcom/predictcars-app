# --- v16h57HO6ZOC_CLEAN_REAL DISCRETE TEMPORAL ATOM AUDITOR BANNER OK ---
...
# ============================================================
# PredictCars V15.7 MAX — BUILD AUDITÁVEL v16h57HO6ZOC_CLEAN_REAL — DISCRETE TEMPORAL ATOM + AUDITOR + BANNER OK
# ============================================================

BUILD_TAG = "v16h57HO6ZOC_REAL_INTRA_LIST_REORGANIZATION_PURO_AUDITOR_OK — INTRA-LIST REORGANIZATION (MAX 3 SWAPS) + AUDITOR OK"
BUILD_REAL_FILE = "app_v15_7_MAX_com_orbita_BUILD_AUDITAVEL_v16h57HO6ZOC_REAL_INTRA_LIST_REORGANIZATION_PURO_AUDITOR_OK.py"
BUILD_CANONICAL_FILE = "app_v15_7_MAX_com_orbita.py"
BUILD_TIME = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
WATERMARK = "2026-04-23_12 (HO6ZOC_REAL_INTRA_LIST_REORGANIZATION_PURO_AUDITOR_OK)"

# ⚠️ st.set_page_config precisa ser a PRIMEIRA chamada Streamlit
st.set_page_config(page_title="PredictCars V15.7 MAX — v16h57HO6ZOC REAL INTRA LIST COHERENCE PURO — BUILD AUDITÁVEL", page_icon="🚗", layout="wide")
...
def pc_v16_packet_final_mount_deep(listas_packet, ranking_vals=None, cp_scores=None, co_matrix=None, n_alvo=6, top_k=10):
    """
    HO6ZOC — REAL_INTRA_LIST_REORGANIZATION_PURO.
    Atua apenas no Top10 já gerado, avaliando a lista como bloco.
    Regras:
    - até 2 trocas coordenadas por lista;
    - não cria lista nova;
    - não altera volume;
    - preserva pelo menos 4 elementos originais;
    - aceita ajuste somente se o score global de coerência do conjunto melhorar.
    """
    try:
        pkt = []
        ...
        if not pkt:
            return listas_packet, {
                ...
                "applied": False,
                "reason": "pacote_vazio",
                "mode": "intra_exec_intra_list_reorganization",
            }
        ...
        def list_score(lst):
            ...
            for i in range(len(vals)):
                for j in range(i + 1, len(vals)):
                    ps = pair_score(vals[i], vals[j])
                    pair_strength += ps
                    pair_scores.append(float(ps))
            ...
            # Coerência de bloco: média dos pares + penalização leve para pares zerados.
            pair_avg = float(pair_strength) / max(1, len(pair_scores))
            zero_pairs = sum(1 for ps in pair_scores if ps <= 0.0)
            zero_pair_penalty = float(zero_pairs) / max(1, len(pair_scores))

            # O score continua usando sinais reais do sistema, mas agora avalia o conjunto completo.
            return float(
                pair_strength * 0.18 +
                pair_avg * 0.22 +
                cp_strength * 0.16 +
                rank_sum * 0.05 -
                spread_penalty * 0.03 -
                zero_pair_penalty * 0.08
            )

        def element_quality(v, others):
            v = int(v)
            others = [int(o) for o in others]
            pair_part = sum(pair_score(v, o) for o in others)
            cp_part = float(cp_scores.get(v, 0.0))
            rank_part = rank_strength(v)
            # Qualidade local do elemento dentro do bloco.
            return float(pair_part * 0.18 + cp_part * 0.16 + rank_part * 0.05)

        top10_base = [list(x) for x in pkt[:top_k]]
        tail = [list(x) for x in pkt[top_k:]]
        before_metrics = packet_metrics(top10_base)

        # Pool fechado: ranking operacional + CP + elementos do próprio pacote/cauda.
        candidate_pool = []

        def _add_candidate(x):
            try:
                ix = int(x)
                if ix > 0 and ix not in candidate_pool:
                    candidate_pool.append(ix)
            except Exception:
                pass

        for v in ranking[:max(24, int(n_alvo) * 4)]:
            _add_candidate(v)

        try:
            for k, _v in sorted(cp_scores.items(), key=lambda kv: (-float(kv[1]), int(kv[0])))[:16]:
                _add_candidate(k)
        except Exception:
            pass

        for lst in tail + top10_base:
        ...
        attempted_total = 0
        ganho_total = 0.0
        max_swaps_per_list = 3

        from itertools import combinations

        for idx, lst in enumerate(top10_base):
            vals = [int(x) for x in lst[:int(n_alvo)]]
            base_score = float(list_score(vals))
            event = {
                "idx": int(idx),
                "lista_original": [int(x) for x in vals],
                "ajuste_aceito": False,
                "removidos": [],
                "adicionados": [],
                "score_antes": round(float(base_score), 6),
                "score_depois": round(float(base_score), 6),
                "ganho": 0.0,
                "motivo": "sem_ajuste",
            }

            if len(vals) != int(n_alvo) or not candidate_pool:
                selected.append(sorted(vals))
                event["motivo"] = "lista_ou_pool_invalido"
                coherence_events.append(event)
                continue

            # Diagnóstico: piores elementos no contexto dos demais.
            qualities = []
            for v in vals:
                others = [x for x in vals if int(x) != int(v)]
                qualities.append((float(element_quality(v, others)), int(v)))
            qualities.sort(key=lambda t: (float(t[0]), int(t[1])))

            weak_candidates = [int(v) for _, v in qualities[:min(4, len(qualities))]]
            event["weak_candidates"] = [int(x) for x in weak_candidates]
            event["weak_quality"] = [round(float(q), 6) for q, _ in qualities[:min(4, len(qualities))]]
            ...
            # Testa 1 troca e 2 trocas coordenadas.
            for swap_qtd in [1, 2]:
                for removed_tuple in combinations(weak_candidates, swap_qtd):
                    removed = [int(x) for x in removed_tuple]
                    kept = [int(x) for x in vals if int(x) not in set(removed)]
                    add_pool = [int(c) for c in candidate_pool if int(c) not in vals]
                    # Limita tentativas para manter leve na nuvem, mas suficiente para bloco local.
                    add_pool = add_pool[:24]
                    for added_tuple in combinations(add_pool, swap_qtd):
                        added = [int(x) for x in added_tuple]
                        cand_list = sorted(dict.fromkeys(kept + added))
                        if len(cand_list) != int(n_alvo):
                            continue
                        # preserva identidade: no mínimo 4 elementos originais.
                        if len(set(cand_list).intersection(set(vals))) < int(n_alvo) - max_swaps_per_list:
                            continue

                        attempted_total += 1
                        cand_score = float(list_score(cand_list))
                        if cand_score > best_score:
                            best_score = cand_score
                            best_list = list(cand_list)
                            best_removed = list(removed)
                            best_added = list(added)
                            best_swap_qtd = int(swap_qtd)

            ganho = float(best_score - base_score)
            if best_swap_qtd > 0 and ganho > 1e-9:
                selected.append(sorted(best_list))
                swaps_total += int(best_swap_qtd)
                ganho_total += ganho
                event.update({
                    "ajuste_aceito": True,
                    "removidos": [int(x) for x in best_removed],
                    "adicionados": [int(x) for x in best_added],
                    "lista_final": [int(x) for x in sorted(best_list)],
                    "score_depois": round(float(best_score), 6),
                    "ganho": round(float(ganho), 6),
                    "swap_qtd": int(best_swap_qtd),
                    "motivo": "ganho_coerencia_bloco",
                })
            else:
                selected.append(sorted(vals))
                event.update({
                    "lista_final": [int(x) for x in sorted(vals)],
                    "swap_qtd": 0,
                    "motivo": "sem_candidato_com_ganho_de_bloco",
                })
        ...
        changed_indices = [int(i) for i, (a, b) in enumerate(zip(top10_base, selected)) if list(a) != list(b)]

        score_base_packet = round(float(sum(list_score(x) for x in top10_base) / max(1, len(top10_base))), 6)
        score_final_packet = round(float(sum(list_score(x) for x in selected) / max(1, len(selected))), 6)
        ganho_medio = round(float(ganho_total / max(1, len(changed_indices))), 6) if changed_indices else 0.0

        return final_packet, {
            "active": True,
            "applied": bool(len(changed_indices) > 0),
            "reason": "ok" if changed_indices else "sem_ajustes_aceitos",
            "mode": "intra_exec_intra_list_reorganization",
            "before_metrics": before_metrics,
            "after_metrics": after_metrics,
            "swaps": int(swaps_total),
            "swaps_total": int(swaps_total),
            "trocas_tentadas": int(attempted_total),
            "trocas_aceitas": int(swaps_total),
            ...
            "ganho_medio": float(ganho_medio),
            "global_adjustment_only": False,
            "intra_list_reorganization_active": True,
            "max_swaps_per_list": int(max_swaps_per_list),
            "candidate_pool_qtd": int(len(candidate_pool)),
            "coherence_events": [dict(x) for x in coherence_events],
...
def pc_v16_new_packet_generator(listas_totais, *, ranking_vals=None, historico_df=None, n_alvo=6, seed=0, max_lists=None):
    try:
        ...
        try:
            st.session_state["v16h57HO6ZOC_generator_call_count"] = int(st.session_state.get("v16h57HO6ZOC_generator_call_count", 0)) + 1
            _steps = st.session_state.get("v16h57HO6ZOC_generator_call_steps")
            if not isinstance(_steps, list):
                _steps = []
            _steps.append({"count": int(st.session_state.get("v16h57HO6ZOC_generator_call_count", 1)), "arg_n": int(len(listas_totais or []))})
            st.session_state["v16h57HO6ZOC_generator_call_steps"] = _steps
        except Exception:
            pass
        base = []
        for lst in (listas_totais or []):
        ...
        try:
            ...
            if cp_scores:
                ...
                # v16h57HO6ZOC_CLEAN_REAL — INJECAO BORDA-PERTO REAL
                ...
                try:
                    ...
                    if inj_candidates:
                        # v16h57HO6ZOC_CLEAN_REAL — injecao mais agressiva: até 3 candidatos subindo até a posição 7
        ...
        # v16h57HO6ZOC_CLEAN_REAL — montagem final profunda para conversão
...
def v16h57FS_clear_mode6_packet_state():
    ...
    keys = [
        ...
        "v16h57HO6ZOC_auditor",
        "v16h57HO6ZOC_generator_call_count",
        "v16h57HO6ZOC_generator_call_steps",
        "v16h57HO6ZOC_pre_sanidade_top10",
        "v16h57HO6ZOC_post_sanidade_top10",
    ]
    try:
        for k in keys:
...
def pc_v16_build_auditor_ho6w(*, npgen_info=None, pre_sanidade_top10=None, post_sanidade_top10=None):
    try:
        ...
        gen_calls = int(st.session_state.get("v16h57HO6ZOC_generator_call_count", 0) or 0)
        changed_pre = bool(npgen_info.get("mudou_no_pacote_final", False))
        fm_active = bool(fm.get("active", False))
        fm_mode_ok = str(fm.get("mode", "")) == "intra_exec_intra_list_reorganization"
        fm_applied = bool(fm.get("applied", False))
        changed_indices = fm.get("changed_indices") or []
        swaps_total = int(fm.get("swaps_total", fm.get("swaps", 0)) or 0)
        ...
        auditor = {
            ...
            "intra_list_reorganization_active": bool(fm.get("intra_list_reorganization_active", False)),
            "swaps_total": int(swaps_total),
            "trocas_tentadas": int(fm.get("trocas_tentadas", 0) or 0),
            "trocas_aceitas": int(fm.get("trocas_aceitas", 0) or 0),
            "ganho_medio": float(fm.get("ganho_medio", 0.0) or 0.0),
            "max_swaps_per_list": int(fm.get("max_swaps_per_list", 0) or 0),
            "candidate_pool_qtd": int(fm.get("candidate_pool_qtd", 0) or 0),
        }
        ...
        if gen_calls != 1:
        ...
        elif not bool(fm.get("intra_list_reorganization_active", False)):
            auditor["status"] = "INVALIDO"
            auditor["motivo"] = "coerencia_intra_lista_inativa"
            ...
            auditor["status"] = "INVALIDO"
            auditor["motivo"] = "coerencia_intra_lista_sem_trocas_aceitas"

        st.session_state["v16h57HO6ZOC_auditor"] = auditor
        return auditor
    except Exception as e:
        auditor = {
            "status": "INVALIDO",
            ...
            "unicidade": "FALHA",
            "generator_call_count": int(st.session_state.get("v16h57HO6ZOC_generator_call_count", 0) or 0),
            "ponto_fluxo": "FALHA",
            "antes_sanidade": "OK",
            "mudou_pacote": "NAO",
            "chegou_top10": "NAO",
            "consistencia_intervencao": "FALHA",
            "intra_list_reorganization_active": False,
            "swaps_total": 0,
        }
        try:
            st.session_state["v16h57HO6ZOC_auditor"] = auditor
...
def pc_resp_aplicar_diversificacao(listas_totais, listas_top10, universo, seed=0, n_alvo=6, memoria_sufocadores=None, cap_pct=0.65, core_min=None):
    ...
    try:
        ...
        # fallback v16h57HO6ZOC_CLEAN_REAL: se nada mudou, força 1 troca mínima na 1a lista do top
        ...
        # v16h57HO6ZOC_CLEAN_REAL safety: guarantee at least one minimal swap if calibration active
        try:
            if trocas == 0 and new_top10:
                base = list(new_top10[0])
                drop = sorted(base)[0]
...
# ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # v16h57HO6ZOC_CLEAN_REAL — CT no fluxo real, antes da sanidade, sem calib_meta
    # ------------------------------------------------------------
    _ranking_vals_dx = []
    ...
    try:
        st.session_state["v16h57HO6ZOC_pre_sanidade_top10"] = [list(lst) for lst in (listas_brutas or [])[:10]]
    except Exception:
        st.session_state["v16h57HO6ZOC_pre_sanidade_top10"] = []
    try:
    ...
    try:
        st.session_state["v16h57HO6ZOC_post_sanidade_top10"] = [list(lst) for lst in (listas_top10 or [])[:10]]
    except Exception:
        st.session_state["v16h57HO6ZOC_post_sanidade_top10"] = []

    try:
        _auditor_ho6 = pc_v16_build_auditor_ho6w(
            npgen_info=(_npgen_dx_info if isinstance(_npgen_dx_info, dict) else {}),
            pre_sanidade_top10=st.session_state.get("v16h57HO6ZOC_pre_sanidade_top10") or [],
            post_sanidade_top10=st.session_state.get("v16h57HO6ZOC_post_sanidade_top10") or [],
        )
    except Exception as _aud_e:
        _auditor_ho6 = {"status": "INVALIDO", "motivo": f"auditor_erro: {_aud_e}"}

    st.markdown("### 🔎 AUDITOR HO6ZOC")
...
# ============================================================
# POST MODO6 AUDIT (v16h57HO6ZOC_CLEAN_REAL)
# ============================================================
try:
    import itertools
    listas_ref = None
...
# ============================================================
# BUILD v16h57HO6ZOC_CLEAN_REAL — CT REAL GENERATOR (PRE-SANIDADE HOOK) + BANNER OK
# CT REAL GENERATOR HOOK (PRE SANIDADE)
# ============================================================
try:
    import streamlit as st
    st.session_state["CT_REAL_GENERATOR_PRE_SANIDADE"] = {
        "build": "v16h57HO6ZOC_CLEAN_REAL",
...
# ============================================================
# BUILD v16h57HO6ZOC_CLEAN_REAL — CT GENERATOR PRE-SANIDADE REAL HOOK + BANNER OK
