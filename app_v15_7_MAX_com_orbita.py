...
def pc_v16_packet_final_mount_deep(listas_packet, ranking_vals=None, cp_scores=None, co_matrix=None, n_alvo=6, top_k=10):
    try:
        ...
        if (
            len(new_top) >= 8
            and int(top_metrics_after_final_push.get("passageiros_unicos", 0)) >= 17
            and float(top_metrics_after_final_push.get("sobreposicao_media", 0.0)) <= 2.60
        ):
            family_freq = {}
            for lst in new_top:
                for v in lst[:int(n_alvo)]:
            ...
            if recurring_vals:
                for idx in range(4, len(new_top)):
                    ...
                    for cand in recurring_vals:
                        if int(cand) not in lst and pair_score(int(cand), preserve) >= 0.90:
                            add = int(cand)
                            break
        ...
        # v16h57FS — ajuste fino de pressão interna pós-recompressão
        # Objetivo: aumentar levemente a coesão útil da família dominante
        # sem voltar ao excesso de compressão.
        fs_applied = False
        fs_swaps = 0
        top_metrics_before_fs = dict(top_metrics_after_recompress)

        if (
            len(new_top) >= 8
            and int(top_metrics_after_recompress.get("passageiros_unicos", 0)) >= 17
            and float(top_metrics_after_recompress.get("sobreposicao_media", 0.0)) <= 2.60
        ):
            family_freq = {}
            for lst in new_top:
                for v in lst[:int(n_alvo)]:
            ...
            if family_core:
                for idx in range(3, min(len(new_top), 9)):
                    lst = list(new_top[idx])
                    preserve = sorted(lst, key=lambda v: (-_fs_score(int(v)), int(v)))[:3]
                    weak = [
                        int(v) for v in sorted(
                            lst,
                            key=lambda v: (_fs_score(int(v)), family_freq.get(int(v), 0), int(v))
                        ) if int(v) not in preserve
                    ]

                    add = None
                    for cand in family_core:
                        if int(cand) not in lst and pair_score(int(cand), preserve) >= 0.95:
                            add = int(cand)
                            break
        ...
        # v16h57FU — micro internal conversion push + core preservation strict
        # Objetivo: empurrar a conversão internamente, preservando o núcleo
        # dominante e evitando qualquer alívio estrutural excessivo.
        fu_applied = False
        fu_swaps = 0
        top_metrics_before_fu = dict(top_metrics_after_fs)

        if (
            len(new_top) >= 8
            and int(top_metrics_after_fs.get("passageiros_unicos", 0)) >= 17
            and float(top_metrics_after_fs.get("sobreposicao_media", 0.0)) >= 2.45
        ):
            family_freq = {}
            for lst in new_top:
                for v in lst[:int(n_alvo)]:
            ...
            def _fu_score(v):
                return (
                    float(cp_scores.get(int(v), 0.0)) * 3.15
                    + float(freq.get(int(v), 0)) * 0.34
                    + float(family_freq.get(int(v), 0)) * 0.72
                    + max(0.0, 1.0 - (ranking_pos.get(int(v), 9999) / max(1, len(ranking_pos) or 1)))
                )

            if family_core and family_push:
                for idx in range(3, min(len(new_top), 9)):
                    lst = list(new_top[idx])
                    preserve = sorted(lst, key=lambda v: (-_fu_score(int(v)), int(v)))[:4]
                    ...
                    weak = [
                        int(v) for v in sorted(
                            lst,
                            key=lambda v: (_fu_score(int(v)), family_freq.get(int(v), 0), int(v))
                        ) if int(v) not in preserve
                    ]
                    if not weak:
                        continue
                    ...
                    if unique_after_trial > int(top_metrics_after_fs.get("passageiros_unicos", 0)):
                        continue
                    if overlap_after_trial < float(top_metrics_after_fs.get("sobreposicao_media", 0.0)):
                        continue
                    if overlap_after_trial > float(top_metrics_after_fs.get("sobreposicao_media", 0.0)) + 0.22:
                        continue
        ...
        top_metrics_after_fu = _packet_metrics(new_top)

        # v16h57FV — final conversion push + structure preserved + micro priority adjust
        # Objetivo: dar um último empurrão de prioridade interna sem abrir o pacote,
        # preservando a família dominante e mantendo a estrutura conquistada.
        fv_applied = False
        fv_swaps = 0
        top_metrics_before_fv = dict(top_metrics_after_fu)

        if (
            len(new_top) >= 8
            and int(top_metrics_after_fu.get("passageiros_unicos", 0)) >= 17
            and float(top_metrics_after_fu.get("sobreposicao_media", 0.0)) >= 2.55
        ):
            family_freq = {}
            for lst in new_top:
                for v in lst[:int(n_alvo)]:
            ...
            def _fv_score(v):
                return (
                    float(cp_scores.get(int(v), 0.0)) * 3.25
                    + float(freq.get(int(v), 0)) * 0.36
                    + float(family_freq.get(int(v), 0)) * 0.82
                    + max(0.0, 1.0 - (ranking_pos.get(int(v), 9999) / max(1, len(ranking_pos) or 1)))
                )

            if family_core and priority_push:
                for idx in range(2, min(len(new_top), 8)):
                    lst = list(new_top[idx])
                    preserve = sorted(lst, key=lambda v: (-_fv_score(int(v)), int(v)))[:4]
                    core_in_preserve = sum(1 for v in preserve if int(v) in family_core)
                    if core_in_preserve < 3:
                        continue
                    ...
                    weak = [
                        int(v) for v in sorted(
                            lst,
                            key=lambda v: (_fv_score(int(v)), family_freq.get(int(v), 0), int(v))
                        ) if int(v) not in preserve
                    ]
                    if not weak:
                        continue
                    ...
                    if unique_after_trial > int(top_metrics_after_fu.get("passageiros_unicos", 0)):
                        continue
                    if overlap_after_trial < float(top_metrics_after_fu.get("sobreposicao_media", 0.0)):
                        continue
                    if overlap_after_trial > float(top_metrics_after_fu.get("sobreposicao_media", 0.0)) + 0.14:
                        continue
        ...
        top_metrics_after_fv = _packet_metrics(new_top)

        # v16h57FW — final micro conversion align + family lock preserved
        # Objetivo: alinhar microsubstituições dentro da família dominante,
        # preservando o lock estrutural e evitando qualquer abertura do envelope.
        fw_applied = False
        fw_swaps = 0
        top_metrics_before_fw = dict(top_metrics_after_fv)

        if (
            len(new_top) >= 8
            and 17 <= int(top_metrics_after_fv.get("passageiros_unicos", 0)) <= 19
            and float(top_metrics_after_fv.get("sobreposicao_media", 0.0)) >= 2.60
        ):
            family_freq = {}
            for lst in new_top:
                for v in lst[:int(n_alvo)]:
            ...
            def _fw_score(v):
                return (
                    float(cp_scores.get(int(v), 0.0)) * 3.30
                    + float(freq.get(int(v), 0)) * 0.38
                    + float(family_freq.get(int(v), 0)) * 0.90
                    + max(0.0, 1.0 - (ranking_pos.get(int(v), 9999) / max(1, len(ranking_pos) or 1)))
                )

            if family_core and family_lock:
                for idx in range(2, min(len(new_top), 7)):
                    lst = list(new_top[idx])
                    preserve = sorted(lst, key=lambda v: (-_fw_score(int(v)), int(v)))[:4]
                    core_in_preserve = sum(1 for v in preserve if int(v) in family_core)
                    if core_in_preserve < 3:
                        continue
                    ...
                    weak = [
                        int(v) for v in sorted(
                            lst,
                            key=lambda v: (_fw_score(int(v)), family_freq.get(int(v), 0), int(v))
                        ) if int(v) not in preserve
                    ]
                    if not weak:
                        continue
                    ...
                    if unique_after_trial > int(top_metrics_after_fv.get("passageiros_unicos", 0)):
                        continue
                    if overlap_after_trial < float(top_metrics_after_fv.get("sobreposicao_media", 0.0)):
                        continue
                    if overlap_after_trial > float(top_metrics_after_fv.get("sobreposicao_media", 0.0)) + 0.08:
                        continue
        ...
        # v16h57HA — POST GZ MICRO CONVERSION FOCUS + INTERNAL PRESSURE ALIGN
        ha_applied = False
        ha_swaps = 0
        top_metrics_before_ha = dict(top_metrics_after_gz)

        if (
            len(new_top) >= 8
            and int(top_metrics_after_gz.get("passageiros_unicos", 0)) >= 19
            and float(top_metrics_after_gz.get("sobreposicao_media", 0.0)) <= 1.90
        ):
            freq_local = {}
            for lst in new_top:
                for v in lst[:int(n_alvo)]:
            ...
            def _ha_score(v):
                return (
                    float(cp_scores.get(int(v), 0.0)) * 3.4
                    + float(freq.get(int(v), 0)) * 0.30
                    + float(freq_local.get(int(v), 0)) * 0.85
                    - (ranking_pos.get(int(v), 9999) * 0.0001)
                )
            ...
            for idx in range(2, min(len(new_top), 8)):
                ...
                preserve = sorted(lst, key=lambda v: (-_ha_score(int(v)), int(v)))[:3]
                weak = [
                    int(v) for v in sorted(
                        lst,
                        key=lambda v: (_ha_score(int(v)), freq_local.get(int(v), 0), int(v))
                    ) if int(v) not in preserve
                ]

                add = None
                for cand in family_focus[:6]:
                    if int(cand) not in lst:
                        add = int(cand)
                        break
                ...
                u_before = int(top_metrics_after_gz.get("passageiros_unicos", 0))
                o_before = float(top_metrics_after_gz.get("sobreposicao_media", 0.0))
        ...
        # v16h57HC — POST HB MICRO CONVERSION SHIFT + INTERNAL PRESSURE FOCUS
        hc_applied = False
        hc_swaps = 0
        top_metrics_before_hc = dict(top_metrics_after_hb)

        if (
            len(new_top) >= 8
            and int(top_metrics_after_hb.get("passageiros_unicos", 0)) >= 19
            and float(top_metrics_after_hb.get("sobreposicao_media", 0.0)) <= 1.85
        ):
            freq_local = {}
            for lst in new_top:
                for v in lst[:int(n_alvo)]:
            ...
            def _hc_score(v):
                return (
                    float(cp_scores.get(int(v), 0.0)) * 3.45
                    + float(freq.get(int(v), 0)) * 0.26
                    + float(freq_local.get(int(v), 0)) * 0.88
                    - (ranking_pos.get(int(v), 9999) * 0.00011)
                )
            ...
            for idx in range(2, min(len(new_top), 7)):
                ...
                preserve = sorted(lst, key=lambda v: (-_hc_score(int(v)), int(v)))[:4]
                weak = [
                    int(v) for v in sorted(
                        lst,
                        key=lambda v: (_hc_score(int(v)), freq_local.get(int(v), 0), int(v))
                    ) if int(v) not in preserve
                ]
                ...
                u_before = int(top_metrics_after_hb.get("passageiros_unicos", 0))
                o_before = float(top_metrics_after_hb.get("sobreposicao_media", 0.0))
                u_after = int(m.get("passageiros_unicos", 0))
                o_after = float(m.get("sobreposicao_media", 0.0))
...
# v16h57GR — POST GQ CONVERSION MICRO LOCK
        ...
        # v16h57GU — POST GT MICRO CONVERSION CENTER
        gu_applied = False
        gu_swaps = 0
        top_metrics_before_gu = dict(top_metrics_after_gt)

        if (
            len(new_top) >= 8
            and 15 <= int(top_metrics_after_gt.get("passageiros_unicos", 0)) <= 19
            and 2.35 <= float(top_metrics_after_gt.get("sobreposicao_media", 0.0)) <= 2.95
        ):
            fam = {}
            for lst in new_top:
                for v in lst[:int(n_alvo)]:
            ...
            fam_center = sorted(fam.keys(), key=lambda v: (-fam[v], int(v)))[:6]

            def _gu_score(v):
                return (
                    float(cp_scores.get(int(v), 0.0)) * 3.1
                    + float(freq.get(int(v), 0)) * 0.30
                    + float(fam.get(int(v), 0)) * 0.85
                )

            for idx in range(3, min(len(new_top), 9)):
                lst = list(new_top[idx])
                keep = sorted(lst, key=lambda v: (-_gu_score(int(v)), int(v)))[:4]
                ...
                weak = [
                    ...
                    if int(v) not in keep
                ]

                add = None
                for cand in fam_center:
                    ic = int(cand)
                    if ic not in lst:
                        add = ic
                        break
                ...
                trial = sorted(
                    dict.fromkeys(
                        [int(v) for v in lst if int(v) != drop] + [int(add)]
                    )
                )[: int(n_alvo)]
                ...
                unique_after = int(tm.get("passageiros_unicos", 0))
                overlap_after = float(tm.get("sobreposicao_media", 0.0))
                unique_before = int(top_metrics_after_gt.get("passageiros_unicos", 0))
                overlap_before = float(top_metrics_after_gt.get("sobreposicao_media", 0.0))
                ...
                if overlap_after < overlap_before:
                    continue
                if overlap_after > overlap_before + 0.08:
                    continue
        ...
        top_metrics_after_gu = _packet_metrics(new_top)


# v16h57FY — final micro conversion lock + rotation slot align + family preserved
        # Objetivo: travar a família já dominante e alinhar apenas 1 slot rotativo
        # sem abrir o envelope e sem recomprimir além do ponto útil.
        fy_applied = False
        fy_swaps = 0
        top_metrics_before_fy = dict(top_metrics_after_fw)

        if (
            len(new_top) >= 8
            and 15 <= int(top_metrics_after_fw.get("passageiros_unicos", 0)) <= 18
            and 2.45 <= float(top_metrics_after_fw.get("sobreposicao_media", 0.0)) <= 2.95
        ):
            family_freq = {}
            for lst in new_top:
                for v in lst[:int(n_alvo)]:
            ...
            def _fy_score(v):
                return (
                    float(cp_scores.get(int(v), 0.0)) * 3.34
            ...
            if family_core and rotation_lock:
                for idx in range(1, min(len(new_top), 6)):
                    ...
                    preserve = sorted(lst, key=lambda v: (-_fy_score(int(v)), int(v)))[:5]
                    core_in_preserve = sum(1 for v in preserve if int(v) in family_core)
                    if core_in_preserve < 3:
                        continue
                    ...
                    weak = [
                        int(v) for v in sorted(
                            lst,
                            key=lambda v: (_fy_score(int(v)), family_freq.get(int(v), 0), int(v))
                        ) if int(v) not in preserve
                    ]
                    if not weak:
                        continue
                    ...
                    if unique_after_trial > int(top_metrics_after_fw.get("passageiros_unicos", 0)):
                        continue
                    if overlap_after_trial < float(top_metrics_after_fw.get("sobreposicao_media", 0.0)):
                        continue
                    if overlap_after_trial > float(top_metrics_after_fw.get("sobreposicao_media", 0.0)) + 0.05:
                        continue
        ...
        if (
            ...
            if family_core and soft_rotation:
                for idx in range(2, min(len(new_top), 5)):
                    ...
                    if unique_after_trial > int(top_metrics_after_fy.get("passageiros_unicos", 0)):
                        continue
        ...
        # v16h57GB — core hold + conversion slot balanced lock + family preserved
        # Objetivo: partir do GA e travar melhor o slot de conversão sem recriar
        # a sobrecompressão observada quando o pacote passa do ponto.
        gb_applied = False
        gb_swaps = 0
        top_metrics_before_gb = dict(top_metrics_after_ga)

        if (
            len(new_top) >= 8
            and 16 <= int(top_metrics_after_ga.get("passageiros_unicos", 0)) <= 18
            and 2.30 <= float(top_metrics_after_ga.get("sobreposicao_media", 0.0)) <= 2.65
        ):
            family_freq = {}
            for lst in new_top:
                for v in lst[:int(n_alvo)]:
            ...
            if family_core and balanced_rotation:
                for idx in range(2, min(len(new_top), 6)):
                    lst = list(new_top[idx])
                    preserve = sorted(lst, key=lambda v: (-_gb_score(int(v)), int(v)))[:4]
                    core_in_preserve = sum(1 for v in preserve if int(v) in family_core)
                    if core_in_preserve < 2:
                        continue
                    ...
                    weak = [
                        int(v) for v in sorted(
                            lst,
                            key=lambda v: (_gb_score(int(v)), family_freq.get(int(v), 0), int(v))
                        ) if int(v) not in preserve
                    ]
                    if not weak:
                        continue
                    ...
                    if unique_after_trial > int(top_metrics_after_ga.get("passageiros_unicos", 0)):
                        continue
                    if unique_after_trial < int(top_metrics_after_ga.get("passageiros_unicos", 0)) - 1:
                        continue
                    if overlap_after_trial < float(top_metrics_after_ga.get("sobreposicao_media", 0.0)):
                        continue
                    if overlap_after_trial > float(top_metrics_after_ga.get("sobreposicao_media", 0.0)) + 0.04:
                        continue
        ...
        # v16h57GD — final micro conversion align + family lock preserved + post GC
        # Objetivo: partir do GC com um alinhamento final ainda mais conservador,
        # mexendo em no máximo 1 slot de 1 lista e preservando a família dominante.
        gd_applied = False
        gd_swaps = 0
        top_metrics_before_gd = dict(top_metrics_after_gb)

        if (
            len(new_top) >= 8
            and 16 <= int(top_metrics_after_gb.get("passageiros_unicos", 0)) <= 18
            and 2.35 <= float(top_metrics_after_gb.get("sobreposicao_media", 0.0)) <= 2.95
        ):
            family_freq = {}
            for lst in new_top:
                for v in lst[:int(n_alvo)]:
            ...
            if family_core and gd_rotation:
                for idx in range(1, min(len(new_top), 5)):
                    lst = list(new_top[idx])
                    preserve = sorted(lst, key=lambda v: (-_gd_score(int(v)), int(v)))[:5]
                    core_in_preserve = sum(1 for v in preserve if int(v) in family_core)
                    if core_in_preserve < 3:
                        continue
                    ...
                    weak = [
                        int(v) for v in sorted(
                            lst,
                            key=lambda v: (_gd_score(int(v)), family_freq.get(int(v), 0), int(v))
                        ) if int(v) not in preserve
                    ]
                    if not weak:
                        continue
                    ...
                    if overlap_after_trial < float(top_metrics_after_gb.get("sobreposicao_media", 0.0)):
                        continue
                    if overlap_after_trial > float(top_metrics_after_gb.get("sobreposicao_media", 0.0)) + 0.04:
                        continue
        ...
        top_metrics_after_gd = _packet_metrics(new_top)

        # v16h57GE — final conversion micro align + slot controlled + family lock preserved
        # Objetivo: após o GD, alinhar apenas 1 slot rotativo em 1 lista,
        # sem abrir o envelope, sem reduzir a pressão e sem tocar na arquitetura.
        ge_applied = False
        ge_swaps = 0
        top_metrics_before_ge = dict(top_metrics_after_gd)

        if (
            len(new_top) >= 8
            and 16 <= int(top_metrics_after_gd.get("passageiros_unicos", 0)) <= 18
            and 2.55 <= float(top_metrics_after_gd.get("sobreposicao_media", 0.0)) <= 3.05
        ):
            family_freq = {}
            for lst in new_top:
                for v in lst[:int(n_alvo)]:
            ...
            def _ge_score(v):
                return (
                    ...
                    + float(freq.get(int(v), 0)) * 0.44
                    + float(family_freq.get(int(v), 0)) * 1.06
                    + max(0.0, 1.0 - (ranking_pos.get(int(v), 9999) / max(1, len(ranking_pos) or 1)))
                )

            if family_core and ge_rotation:
                for idx in range(1, min(len(new_top), 4)):
                    lst = list(new_top[idx])
                    preserve = sorted(lst, key=lambda v: (-_ge_score(int(v)), int(v)))[:5]
                    core_in_preserve = sum(1 for v in preserve if int(v) in family_core)
                    if core_in_preserve < 3:
                        continue
                    ...
                    weak = [
                        int(v) for v in sorted(
                            lst,
                            key=lambda v: (_ge_score(int(v)), family_freq.get(int(v), 0), int(v))
                        ) if int(v) not in preserve
                    ]
                    if not weak:
                        continue
                    ...
                    if unique_after_trial > int(top_metrics_after_gd.get("passageiros_unicos", 0)):
                        continue
                    if overlap_after_trial < float(top_metrics_after_gd.get("sobreposicao_media", 0.0)):
                        continue
                    if overlap_after_trial > float(top_metrics_after_gd.get("sobreposicao_media", 0.0)) + 0.03:
                        continue
        ...
        top_metrics_after_ge = _packet_metrics(new_top)

        # v16h57GF — final conversion micro stabilization + slot lock + family lock preserved
        # Objetivo: após o GE, estabilizar o slot dominante sem abrir o envelope
        # e sem perder a família que já vinha convergindo.
        gf_applied = False
        gf_swaps = 0
        top_metrics_before_gf = dict(top_metrics_after_ge)

        if (
            len(new_top) >= 8
            and 17 <= int(top_metrics_after_ge.get("passageiros_unicos", 0)) <= 19
            and 2.50 <= float(top_metrics_after_ge.get("sobreposicao_media", 0.0)) <= 2.95
        ):
            family_freq = {}
            for lst in new_top:
                for v in lst[:int(n_alvo)]:
            ...
            def _gf_score(v):
                return (
                    float(cp_scores.get(int(v), 0.0)) * 3.48
                    + float(freq.get(int(v), 0)) * 0.46
                    + float(family_freq.get(int(v), 0)) * 1.14
                    + max(0.0, 1.0 - (ranking_pos.get(int(v), 9999) / max(1, len(ranking_pos) or 1)))
                )

            if family_core and stable_slot:
                for idx in range(1, min(len(new_top), 4)):
                    lst = list(new_top[idx])
                    preserve = sorted(lst, key=lambda v: (-_gf_score(int(v)), int(v)))[:5]
                    core_in_preserve = sum(1 for v in preserve if int(v) in family_core)
                    if core_in_preserve < 3:
                        continue
                    ...
                    weak = [
                        int(v) for v in sorted(
                            lst,
                            key=lambda v: (_gf_score(int(v)), family_freq.get(int(v), 0), int(v))
                        ) if int(v) not in preserve
                    ]
                    if not weak:
                        continue
                    ...
                    if unique_after_trial > int(top_metrics_after_ge.get("passageiros_unicos", 0)):
                        continue
                    if overlap_after_trial < float(top_metrics_after_ge.get("sobreposicao_media", 0.0)):
                        continue
                    if overlap_after_trial > float(top_metrics_after_ge.get("sobreposicao_media", 0.0)) + 0.02:
                        continue
        ...
        if (
            ...
            if family_core and repress_candidates:
                for idx in range(2, min(len(new_top), 7)):
                    ...
                    add = None
                    for cand in repress_candidates:
                        if int(cand) in lst:
                            continue
                        local_pair = pair_score(int(cand), preserve[:3])
                        if local_pair >= 1.03:
                    ...
                    if unique_after_trial > int(top_metrics_after_gf.get("passageiros_unicos", 0)):
                        continue
        ...
        # v16h57GK — final conversion micro recompress + post CT family lock preserved
        # Objetivo: recuperar leve pressão interna após o micro-fit,
        # sem reabrir o envelope e sem colapsar a família dominante.
        gk_applied = False
        gk_swaps = 0
        top_metrics_before_gk = dict(top_metrics_after_gj)

        if (
            len(new_top) >= 8
            and 18 <= int(top_metrics_after_gj.get("passageiros_unicos", 0)) <= 20
            and float(top_metrics_after_gj.get("sobreposicao_media", 0.0)) <= 2.35
        ):
            family_freq = {}
            for lst in new_top:
                for v in lst[:int(n_alvo)]:
            ...
            if family_core and repress_candidates:
                for idx in range(2, min(len(new_top), 7)):
                    lst = list(new_top[idx])
                    preserve = sorted(lst, key=lambda v: (-_gk_score(int(v)), int(v)))[:4]
                    core_in_preserve = sum(1 for v in preserve if int(v) in family_core)
                    if core_in_preserve < 3:
                        continue
                    ...
                    weak = [
                        int(v) for v in sorted(
                            lst,
                            key=lambda v: (_gk_score(int(v)), family_freq.get(int(v), 0), int(v))
                        ) if int(v) not in preserve
                    ]
                    if not weak:
                        continue
                    ...
                    if unique_after_trial > int(top_metrics_after_gj.get("passageiros_unicos", 0)):
                        continue
                    if overlap_after_trial < float(top_metrics_after_gj.get("sobreposicao_media", 0.0)):
                        continue
                    if overlap_after_trial > float(top_metrics_after_gj.get("sobreposicao_media", 0.0)) + 0.12:
                        continue
        ...
        top_metrics_after_gk = _packet_metrics(new_top)

        # v16h57GL — final conversion micro balance + post recompress family lock preserved
        # Objetivo: equilibrar levemente o slot variável após a recompressão,
        # mantendo a família dominante estável sem voltar a abrir o envelope.
        gl_applied = False
        gl_swaps = 0
        top_metrics_before_gl = dict(top_metrics_after_gk)

        if (
            len(new_top) >= 8
            and 17 <= int(top_metrics_after_gk.get("passageiros_unicos", 0)) <= 19
            and 2.35 <= float(top_metrics_after_gk.get("sobreposicao_media", 0.0)) <= 2.75
        ):
            family_freq = {}
            for lst in new_top:
                for v in lst[:int(n_alvo)]:
            ...
            def _gl_score(v):
                return (
                    float(cp_scores.get(int(v), 0.0)) * 3.40
                    + float(freq.get(int(v), 0)) * 0.40
                    + float(family_freq.get(int(v), 0)) * 0.95
                    + max(0.0, 1.0 - (ranking_pos.get(int(v), 9999) / max(1, len(ranking_pos) or 1)))
                )

            if family_core and balance_candidates:
                for idx in range(3, min(len(new_top), 8)):
                    lst = list(new_top[idx])
                    preserve = sorted(lst, key=lambda v: (-_gl_score(int(v)), int(v)))[:4]
                    core_in_preserve = sum(1 for v in preserve if int(v) in family_core)
                    if core_in_preserve < 3:
                        continue
                    ...
                    weak = [
                        int(v) for v in sorted(
                            lst,
                            key=lambda v: (_gl_score(int(v)), family_freq.get(int(v), 0), int(v))
                        ) if int(v) not in preserve
                    ]
                    if not weak:
                        continue
                    ...
                    if unique_after_trial > int(top_metrics_after_gk.get("passageiros_unicos", 0)) + 1:
                        continue
                    if overlap_after_trial < float(top_metrics_after_gk.get("sobreposicao_media", 0.0)) - 0.10:
                        continue
                    if overlap_after_trial > float(top_metrics_after_gk.get("sobreposicao_media", 0.0)) + 0.10:
                        continue
        ...
        # v16h57GO — micro rotation internal post GN + conversion push controlled
        # Objetivo: liberar micro-mobilidade dentro da família dominante
        # sem abrir a borda externa nem quebrar o lock estrutural já conquistado.
        go_applied = False
        go_swaps = 0
        top_metrics_before_go = dict(top_metrics_after_gm)

        if (
            len(new_top) >= 8
            and 17 <= int(top_metrics_after_gm.get("passageiros_unicos", 0)) <= 19
            and 2.35 <= float(top_metrics_after_gm.get("sobreposicao_media", 0.0)) <= 2.80
        ):
            family_freq = {}
            for lst in new_top:
                for v in lst[:int(n_alvo)]:
            ...
            def _go_score(v):
                return (
                    float(cp_scores.get(int(v), 0.0)) * 3.00
                    + float(freq.get(int(v), 0)) * 0.28
                    + float(family_freq.get(int(v), 0)) * 0.52
                    + max(0.0, 1.0 - (ranking_pos.get(int(v), 9999) / max(1, len(ranking_pos) or 1)))
                )

            if family_core and family_rot:
                # Atua apenas em 1 lista do meio do Top10 para criar rotação interna mínima
                for idx in range(3, min(len(new_top), 8)):
                    lst = list(new_top[idx])
                    preserve = sorted(lst, key=lambda v: (-_go_score(int(v)), int(v)))[:3]
                    core_in_preserve = sum(1 for v in preserve if int(v) in family_core)
                    if core_in_preserve < 2:
                        continue
                    ...
                    weak = [
                        int(v) for v in sorted(
                            lst,
                            key=lambda v: (_go_score(int(v)), family_freq.get(int(v), 0), int(v))
                        ) if int(v) not in preserve
                    ]
                    if not weak:
                        continue
                    ...
                    add = None
                    for cand in family_rot:
                        ic = int(cand)
                        if ic in lst or ic in preserve:
                            continue
                        local_pair = pair_score(ic, preserve)
                        if 0.92 <= local_pair <= 1.28:
                            add = ic
                            break
                    if add is None:
                        continue
                    ...
                    # micro-rotação: não abrir demais e não recomprimir demais
                    if unique_after_trial < int(top_metrics_after_gm.get("passageiros_unicos", 0)):
                        continue
                    if unique_after_trial > int(top_metrics_after_gm.get("passageiros_unicos", 0)) + 1:
                        continue
                    if overlap_after_trial < float(top_metrics_after_gm.get("sobreposicao_media", 0.0)) - 0.12:
                        continue
                    if overlap_after_trial > float(top_metrics_after_gm.get("sobreposicao_media", 0.0)) + 0.10:
                        continue
        ...
        top_metrics_after_go = _packet_metrics(new_top)


        # v16h57GP — post GO micro lock + conversion stabilizer
        # Objetivo: estabilizar a rotação interna criada pelo GO, sem reabrir o envelope
        # e sem voltar à compressão excessiva.
        gp_applied = False
        gp_swaps = 0
        top_metrics_before_gp = dict(top_metrics_after_go)

        if (
            len(new_top) >= 8
            and 16 <= int(top_metrics_after_go.get("passageiros_unicos", 0)) <= 18
            and 2.40 <= float(top_metrics_after_go.get("sobreposicao_media", 0.0)) <= 2.75
        ):
            family_freq = {}
            for lst in new_top:
                for v in lst[:int(n_alvo)]:
            ...
            def _gp_score(v):
                return (
                    float(cp_scores.get(int(v), 0.0)) * 3.08
                    + float(freq.get(int(v), 0)) * 0.31
                    + float(family_freq.get(int(v), 0)) * 0.66
                    + max(0.0, 1.0 - (ranking_pos.get(int(v), 9999) / max(1, len(ranking_pos) or 1)))
                )
            ...
            if family_core and family_lock:
                for idx in range(2, min(len(new_top), 8)):
                    lst = list(new_top[idx])
                    preserve = sorted(lst, key=lambda v: (-_gp_score(int(v)), int(v)))[:4]
                    core_in_preserve = sum(1 for v in preserve if int(v) in family_core)
                    if core_in_preserve < 3:
                        continue
                    ...
                    weak = [
                        int(v) for v in sorted(
                            lst,
                            key=lambda v: (_gp_score(int(v)), family_freq.get(int(v), 0), int(v))
                        ) if int(v) not in preserve
                    ]
                    if not weak:
                        continue
                    ...
                    for cand in family_lock:
                        ...
                        if ic in lst:
                            continue
                        local_pair = pair_score(ic, preserve[:3])
                        if local_pair >= 0.98:
                            add = ic
                            break
                    if add is None:
                    ...
                    if unique_after_trial < int(top_metrics_after_go.get("passageiros_unicos", 0)) - 1:
                        continue
                    if unique_after_trial > int(top_metrics_after_go.get("passageiros_unicos", 0)):
                        continue
                    if overlap_after_trial < float(top_metrics_after_go.get("sobreposicao_media", 0.0)):
                        continue
                    if overlap_after_trial > float(top_metrics_after_go.get("sobreposicao_media", 0.0)) + 0.12:
                        continue
        ...
        top_metrics_after_gp = _packet_metrics(new_top)
        ...
        return out, {
            ...
            "go_applied": bool(go_applied),
            "go_swaps": int(go_swaps),
            "top_unique_before_go": int(top_metrics_before_go.get("passageiros_unicos", 0)),
            "top_overlap_before_go": float(top_metrics_before_go.get("sobreposicao_media", 0.0)),
            "top_unique_after_go": int(top_metrics_after_go.get("passageiros_unicos", 0)),
            "top_overlap_after_go": float(top_metrics_after_go.get("sobreposicao_media", 0.0)),
            "gp_applied": bool(gp_applied),
            "gp_swaps": int(gp_swaps),
            "top_unique_before_gp": int(top_metrics_before_gp.get("passageiros_unicos", 0)),
            "top_overlap_before_gp": float(top_metrics_before_gp.get("sobreposicao_media", 0.0)),
            "top_unique_after_gp": int(top_metrics_after_gp.get("passageiros_unicos", 0)),
            "top_overlap_after_gp": float(top_metrics_after_gp.get("sobreposicao_media", 0.0)),
            ...
            "fs_applied": bool(fs_applied),
            "fs_swaps": int(fs_swaps),
            "top_unique_before_fs": int(top_metrics_before_fs.get("passageiros_unicos", 0)),
            ...
            "fu_applied": bool(fu_applied),
            "fu_swaps": int(fu_swaps),
            "top_unique_before_fu": int(top_metrics_before_fu.get("passageiros_unicos", 0)),
            "top_overlap_before_fu": float(top_metrics_before_fu.get("sobreposicao_media", 0.0)),
            "top_unique_after_fu": int(top_metrics_after_fu.get("passageiros_unicos", 0)),
            "top_overlap_after_fu": float(top_metrics_after_fu.get("sobreposicao_media", 0.0)),
            "fv_applied": bool(fv_applied),
            "fv_swaps": int(fv_swaps),
            "top_unique_before_fv": int(top_metrics_before_fv.get("passageiros_unicos", 0)),
            "top_overlap_before_fv": float(top_metrics_before_fv.get("sobreposicao_media", 0.0)),
            "top_unique_after_fv": int(top_metrics_after_fv.get("passageiros_unicos", 0)),
            "top_overlap_after_fv": float(top_metrics_after_fv.get("sobreposicao_media", 0.0)),
            "fw_applied": bool(fw_applied),
            "fw_swaps": int(fw_swaps),
            "top_unique_before_fw": int(top_metrics_before_fw.get("passageiros_unicos", 0)),
            "top_overlap_before_fw": float(top_metrics_before_fw.get("sobreposicao_media", 0.0)),
            "top_unique_after_fw": int(top_metrics_after_fw.get("passageiros_unicos", 0)),
            "top_overlap_after_fw": float(top_metrics_after_fw.get("sobreposicao_media", 0.0)),

            "fy_applied": bool(fy_applied),
            "fy_swaps": int(fy_swaps),
            "top_unique_before_fy": int(top_metrics_before_fy.get("passageiros_unicos", 0)),
            ...
            "ga_applied": bool(ga_applied),
            "ga_swaps": int(ga_swaps),
            "top_unique_before_ga": int(top_metrics_before_ga.get("passageiros_unicos", 0)),
            "top_overlap_before_ga": float(top_metrics_before_ga.get("sobreposicao_media", 0.0)),
            "top_unique_after_ga": int(top_metrics_after_ga.get("passageiros_unicos", 0)),
            "top_overlap_after_ga": float(top_metrics_after_ga.get("sobreposicao_media", 0.0)),

            "gb_applied": bool(gb_applied),
            "gb_swaps": int(gb_swaps),
            "top_unique_before_gb": int(top_metrics_before_gb.get("passageiros_unicos", 0)),
            ...
            "gd_applied": bool(gd_applied),
            "gd_swaps": int(gd_swaps),
            "top_unique_before_gd": int(top_metrics_before_gd.get("passageiros_unicos", 0)),
            "top_overlap_before_gd": float(top_metrics_before_gd.get("sobreposicao_media", 0.0)),
            "top_unique_after_gd": int(top_metrics_after_gd.get("passageiros_unicos", 0)),
            "top_overlap_after_gd": float(top_metrics_after_gd.get("sobreposicao_media", 0.0)),

            "ge_applied": bool(ge_applied),
            "ge_swaps": int(ge_swaps),
            "top_unique_before_ge": int(top_metrics_before_ge.get("passageiros_unicos", 0)),
            "top_overlap_before_ge": float(top_metrics_before_ge.get("sobreposicao_media", 0.0)),
            "top_unique_after_ge": int(top_metrics_after_ge.get("passageiros_unicos", 0)),
            "top_overlap_after_ge": float(top_metrics_after_ge.get("sobreposicao_media", 0.0)),

            "gf_applied": bool(gf_applied),
            "gf_swaps": int(gf_swaps),
            "top_unique_before_gf": int(top_metrics_before_gf.get("passageiros_unicos", 0)),
            "top_overlap_before_gf": float(top_metrics_before_gf.get("sobreposicao_media", 0.0)),
            ...
            "gk_applied": bool(gk_applied),
            "gk_swaps": int(gk_swaps),
            "top_unique_before_gk": int(top_metrics_before_gk.get("passageiros_unicos", 0)),
            "top_overlap_before_gk": float(top_metrics_before_gk.get("sobreposicao_media", 0.0)),
            "top_unique_after_gk": int(top_metrics_after_gk.get("passageiros_unicos", 0)),
            "top_overlap_after_gk": float(top_metrics_after_gk.get("sobreposicao_media", 0.0)),

            "gl_applied": bool(gl_applied),
            "gl_swaps": int(gl_swaps),
            "top_unique_before_gl": int(top_metrics_before_gl.get("passageiros_unicos", 0)),
