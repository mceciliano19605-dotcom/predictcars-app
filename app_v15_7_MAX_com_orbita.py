# --- v16h57HO6ZOB_CLEAN_REAL DISCRETE TEMPORAL ATOM AUDITOR BANNER OK ---
...
# ============================================================
# PredictCars V15.7 MAX — BUILD AUDITÁVEL v16h57HO6ZOB_CLEAN_REAL — DISCRETE TEMPORAL ATOM + AUDITOR + BANNER OK
# ============================================================

BUILD_TAG = "v16h57HO6ZOB_REAL_INTRA_LIST_COHERENCE_PURO_AUDITOR_OK — INTRA-LIST COHERENCE (MAX 2 SWAPS) + AUDITOR OK"
BUILD_REAL_FILE = "app_v15_7_MAX_com_orbita_BUILD_AUDITAVEL_v16h57HO6ZOB_REAL_INTRA_LIST_COHERENCE_PURO_AUDITOR_OK.py"
BUILD_CANONICAL_FILE = "app_v15_7_MAX_com_orbita.py"
BUILD_TIME = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
WATERMARK = "2026-04-23_12 (HO6ZOB_REAL_INTRA_LIST_COHERENCE_PURO_AUDITOR_OK)"

# ⚠️ st.set_page_config precisa ser a PRIMEIRA chamada Streamlit
st.set_page_config(page_title="PredictCars V15.7 MAX — v16h57HO6ZOB REAL INTRA LIST COHERENCE PURO — BUILD AUDITÁVEL", page_icon="🚗", layout="wide")
...
def pc_v16_packet_final_mount_deep(listas_packet, ranking_vals=None, cp_scores=None, co_matrix=None, n_alvo=6, top_k=10):
    """
    HO6ZOB — REAL_INTRA_LIST_COHERENCE_PURO.
    ...
    try:
        ...
        if not pkt:
            return listas_packet, {
                ...
                "applied": False,
                "reason": "pacote_vazio",
                "mode": "intra_exec_intra_list_coherence",
            }
        ...
        return final_packet, {
            ...
            "intra_list_coherence_active": True,
...
def pc_v16_new_packet_generator(listas_totais, *, ranking_vals=None, historico_df=None, n_alvo=6, seed=0, max_lists=None):
    try:
        ...
        try:
            st.session_state["v16h57HO6ZOB_generator_call_count"] = int(st.session_state.get("v16h57HO6ZOB_generator_call_count", 0)) + 1
            _steps = st.session_state.get("v16h57HO6ZOB_generator_call_steps")
            if not isinstance(_steps, list):
                _steps = []
            _steps.append({"count": int(st.session_state.get("v16h57HO6ZOB_generator_call_count", 1)), "arg_n": int(len(listas_totais or []))})
            st.session_state["v16h57HO6ZOB_generator_call_steps"] = _steps
        except Exception:
            pass
        base = []
        for lst in (listas_totais or []):
        ...
        try:
            ...
            if cp_scores:
                ...
                # v16h57HO6ZOB_CLEAN_REAL — INJECAO BORDA-PERTO REAL
                ...
                try:
                    ...
                    if inj_candidates:
                        # v16h57HO6ZOB_CLEAN_REAL — injecao mais agressiva: até 3 candidatos subindo até a posição 7
        ...
        # v16h57HO6ZOB_CLEAN_REAL — montagem final profunda para conversão
...
def v16h57FS_clear_mode6_packet_state():
    ...
    keys = [
        ...
        "v16h57HO6ZOB_auditor",
        "v16h57HO6ZOB_generator_call_count",
        "v16h57HO6ZOB_generator_call_steps",
        "v16h57HO6ZOB_pre_sanidade_top10",
        "v16h57HO6ZOB_post_sanidade_top10",
    ]
    try:
        for k in keys:
...
def pc_v16_build_auditor_ho6w(*, npgen_info=None, pre_sanidade_top10=None, post_sanidade_top10=None):
    try:
        ...
        gen_calls = int(st.session_state.get("v16h57HO6ZOB_generator_call_count", 0) or 0)
        changed_pre = bool(npgen_info.get("mudou_no_pacote_final", False))
        fm_active = bool(fm.get("active", False))
        fm_mode_ok = str(fm.get("mode", "")) == "intra_exec_intra_list_coherence"
        ...
        auditor = {
            ...
            "intra_list_coherence_active": bool(fm.get("intra_list_coherence_active", False)),
            "swaps_total": int(swaps_total),
        ...
        elif not bool(fm.get("intra_list_coherence_active", False)):
            auditor["status"] = "INVALIDO"
        ...
        st.session_state["v16h57HO6ZOB_auditor"] = auditor
        return auditor
    except Exception as e:
        auditor = {
            "status": "INVALIDO",
            ...
            "unicidade": "FALHA",
            "generator_call_count": int(st.session_state.get("v16h57HO6ZOB_generator_call_count", 0) or 0),
            "ponto_fluxo": "FALHA",
            "antes_sanidade": "OK",
            "mudou_pacote": "NAO",
            "chegou_top10": "NAO",
            "consistencia_intervencao": "FALHA",
            "intra_list_coherence_active": False,
            "swaps_total": 0,
        }
        try:
            st.session_state["v16h57HO6ZOB_auditor"] = auditor
...
def pc_resp_aplicar_diversificacao(listas_totais, listas_top10, universo, seed=0, n_alvo=6, memoria_sufocadores=None, cap_pct=0.65, core_min=None):
    ...
    try:
        ...
        # fallback v16h57HO6ZOB_CLEAN_REAL: se nada mudou, força 1 troca mínima na 1a lista do top
        ...
        # v16h57HO6ZOB_CLEAN_REAL safety: guarantee at least one minimal swap if calibration active
        try:
            if trocas == 0 and new_top10:
                base = list(new_top10[0])
                drop = sorted(base)[0]
...
# ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # v16h57HO6ZOB_CLEAN_REAL — CT no fluxo real, antes da sanidade, sem calib_meta
    # ------------------------------------------------------------
    _ranking_vals_dx = []
    ...
    try:
        st.session_state["v16h57HO6ZOB_pre_sanidade_top10"] = [list(lst) for lst in (listas_brutas or [])[:10]]
    except Exception:
        st.session_state["v16h57HO6ZOB_pre_sanidade_top10"] = []
    try:
    ...
    try:
        st.session_state["v16h57HO6ZOB_post_sanidade_top10"] = [list(lst) for lst in (listas_top10 or [])[:10]]
    except Exception:
        st.session_state["v16h57HO6ZOB_post_sanidade_top10"] = []

    try:
        _auditor_ho6 = pc_v16_build_auditor_ho6w(
            npgen_info=(_npgen_dx_info if isinstance(_npgen_dx_info, dict) else {}),
            pre_sanidade_top10=st.session_state.get("v16h57HO6ZOB_pre_sanidade_top10") or [],
            post_sanidade_top10=st.session_state.get("v16h57HO6ZOB_post_sanidade_top10") or [],
        )
    except Exception as _aud_e:
        _auditor_ho6 = {"status": "INVALIDO", "motivo": f"auditor_erro: {_aud_e}"}

    st.markdown("### 🔎 AUDITOR HO6ZOB")
...
# ============================================================
# POST MODO6 AUDIT (v16h57HO6ZOB_CLEAN_REAL)
# ============================================================
try:
    import itertools
    listas_ref = None
...
# ============================================================
# BUILD v16h57HO6ZOB_CLEAN_REAL — CT REAL GENERATOR (PRE-SANIDADE HOOK) + BANNER OK
# CT REAL GENERATOR HOOK (PRE SANIDADE)
# ============================================================
try:
    import streamlit as st
    st.session_state["CT_REAL_GENERATOR_PRE_SANIDADE"] = {
        "build": "v16h57HO6ZOB_CLEAN_REAL",
...
# ============================================================
# BUILD v16h57HO6ZOB_CLEAN_REAL — CT GENERATOR PRE-SANIDADE REAL HOOK + BANNER OK
