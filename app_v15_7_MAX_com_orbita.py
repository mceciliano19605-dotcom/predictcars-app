...
BUILD_TAG = "v16h57H8M_E_NEGATIVE_RUNTIME_SAMPLES"
BUILD_REAL_FILE = "app_v16h57H8M_E_NEGATIVE_RUNTIME_SAMPLES.py"
BUILD_CANONICAL_FILE = "app_v15_7_MAX_com_orbita.py"
BUILD_TIME = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
WATERMARK = "BUILD: v16h57H8M_E_NEGATIVE_RUNTIME_SAMPLES"

# ⚠️ st.set_page_config precisa ser a PRIMEIRA chamada Streamlit
st.set_page_config(page_title="PredictCars V15.7 MAX — v16h57H8M_E_NEGATIVE_RUNTIME_SAMPLES")
...
# ============================================================
# V16h57HO6ZOY — HISTORICAL CLOSURE EVIDENCE (REAL 4+)
# Base fixa por rodada: padrões históricos que de fato antecederam 4+.
# Não usa snapshot_p0 dinâmico; não usa proxy de pares/forma.
# ============================================================
def pc_v16_build_historical_patterns_4p(series_hist, n_alvo=6):
    try:
        from itertools import combinations
        patterns = {}
        hist = []
        for s in (series_hist or []):
            try:
                vals = sorted(dict.fromkeys(int(x) for x in list(s)[:int(n_alvo)]))
                if len(vals) == int(n_alvo):
                    hist.append(vals)
            except Exception:
                pass
        if len(hist) < 2:
            return {"ok": False, "patterns": {}, "total_series": len(hist), "motivo": "historico_insuficiente"}

        for idx in range(1, len(hist)):
            prev_vals = hist[idx - 1]
            target_vals = set(hist[idx])
            for r in (3, 4):
                for sub in combinations(prev_vals, r):
                    key = tuple(sorted(int(x) for x in sub))
                    rec = patterns.setdefault(key, {"occ": 0, "hit4": 0})
                    rec["occ"] += 1
                    overlap = len(set(key).intersection(target_vals))
                    if overlap >= min(4, len(key)):
                        rec["hit4"] += 1

        return {"ok": True, "patterns": patterns, "total_series": len(hist), "subsets_total": int(len(patterns)), "motivo": "ok"}
    except Exception as e:
        return {"ok": False, "patterns": {}, "total_series": 0, "motivo": f"erro_patterns_4p: {e}"}


def pc_v16_historical_closure_evidence(candidate, current, historical_patterns_4p=None, n_alvo=6, min_occurrences=15):
    try:
        from itertools import combinations
        hp = historical_patterns_4p if isinstance(historical_patterns_4p, dict) else {}
        patterns = hp.get("patterns", hp) if isinstance(hp, dict) else {}
        if not isinstance(patterns, dict) or not patterns:
            return {"score_raw": 0.0, "valid": False, "subsets_tested": 0, "subsets_valid": 0, "occ_total": 0, "hit4_total": 0, "samples": [], "motivo": "sem_patterns"}

        vals = sorted(dict.fromkeys(int(x) for x in list(current) + [int(candidate)]))
        if len(vals) < 3:
            return {"score_raw": 0.0, "valid": False, "subsets_tested": 0, "subsets_valid": 0, "occ_total": 0, "hit4_total": 0, "samples": [], "motivo": "grupo_menor_que_3"}

        weighted_scores, weights = [], []
        subsets_tested = subsets_valid = occ_total = hit4_total = 0
        samples = []
        for r in (3, 4):
            if len(vals) < r:
                continue
            for sub in combinations(vals, r):
                if int(candidate) not in sub:
                    continue
                key = tuple(sorted(int(x) for x in sub))
                subsets_tested += 1
                rec = patterns.get(key)
                if not isinstance(rec, dict):
                    continue
                occ = int(rec.get("occ", 0) or 0)
                hit4 = int(rec.get("hit4", 0) or 0)
                if occ < int(min_occurrences):
                    continue
                subsets_valid += 1
                occ_total += occ
                hit4_total += hit4
                rate = float(hit4) / max(1.0, float(occ))
                confidence = min(1.0, float(occ) / 50.0)
                weight = float(np.log(float(occ) + 1.0))
                score_sub = float(rate * confidence)
                weighted_scores.append(score_sub * weight)
                weights.append(weight)
                if len(samples) < 12:
                    samples.append({"subset": [int(x) for x in key], "occ": occ, "hit4": hit4, "rate": round(rate, 6), "confidence": round(confidence, 6), "score_sub": round(score_sub, 6)})

        if not weighted_scores:
            return {"score_raw": -0.15, "valid": False, "subsets_tested": int(subsets_tested), "subsets_valid": 0, "occ_total": int(occ_total), "hit4_total": int(hit4_total), "samples": samples, "motivo": "sem_subconjunto_valido"}

        score_raw = float(sum(weighted_scores) / max(1e-9, sum(weights)))
        return {"score_raw": score_raw, "valid": True, "subsets_tested": int(subsets_tested), "subsets_valid": int(subsets_valid), "occ_total": int(occ_total), "hit4_total": int(hit4_total), "samples": samples, "motivo": "ok"}
    except Exception as e:
        return {"score_raw": 0.0, "valid": False, "subsets_tested": 0, "subsets_valid": 0, "occ_total": 0, "hit4_total": 0, "samples": [], "motivo": f"erro_evidence: {e}"}


# ============================================================
# V16h57HO6ZOY — FULL SET GLOBAL SELECTION LAYER
# lista -> full_set_score -> seleção/ranking -> pacote pré-sanidade
# ============================================================
def pc_v16_full_set_global_selection_layer(listas, co_matrix=None, target_profile=None, n_alvo=6, top_n=12):
    try:
        ...
        if not base:
            return listas, {
                ...
                "applied": False,
                "reason": "base_vazia",
                "mode": "full_set_global_selection_pre_sanidade",
        ...
        def historical_alignment(vals):
            try:
                if not (isinstance(target_profile, dict) and bool(target_profile.get("ok"))):
                    return 0.0
                vals = sorted(int(x) for x in vals[:int(n_alvo)])
                spread = max(vals) - min(vals)
                low = sum(1 for x in vals if int(x) <= 17)
                mid = sum(1 for x in vals if 18 <= int(x) <= 34)
                high = sum(1 for x in vals if int(x) >= 35)
                even = sum(1 for x in vals if int(x) % 2 == 0)
                consec = 0
                for i in range(len(vals) - 1):
                    if int(vals[i + 1]) - int(vals[i]) == 1:
                        consec += 1
                err = (
                    abs(float(spread) - float(target_profile.get("spread_mean", 0.0))) / 50.0
                    + abs(float(low) - float(target_profile.get("low_mean", 0.0))) / max(1.0, float(n_alvo))
                    + abs(float(mid) - float(target_profile.get("mid_mean", 0.0))) / max(1.0, float(n_alvo))
                    + abs(float(high) - float(target_profile.get("high_mean", 0.0))) / max(1.0, float(n_alvo))
                    + abs(float(even) - float(target_profile.get("even_mean", 0.0))) / max(1.0, float(n_alvo))
                    + abs(float(consec) - float(target_profile.get("consecutive_mean", 0.0))) / max(1.0, float(n_alvo))
                )
                return max(0.0, 1.0 - float(err) / 6.0)
            except Exception:
                return 0.0

        def full_set_score(vals):
            vals = sorted(dict.fromkeys(int(x) for x in list(vals)[:int(n_alvo)]))
            if len(vals) != int(n_alvo):
                return -1e9
            pair_scores = []
            ...
            pair_strength = sum(float(s) for s in pair_scores) / max(1, len(pair_scores))
            pair_consistency = sum(1 for s in pair_scores if float(s) > 0.0) / max(1, len(pair_scores))
            structural_conflict = sum(1 for s in pair_scores if float(s) <= 0.0) / max(1, len(pair_scores))
            spread = max(vals) - min(vals)
            spread_penalty = min(float(spread) / 60.0, 1.0)
            structure_density = max(0.0, 1.0 - abs(float(spread_penalty) - 0.42))
            hist_align = historical_alignment(vals)
            return float(
                pair_strength * 0.25
                + pair_consistency * 0.20
                + structure_density * 0.15
                + hist_align * 0.20
                - structural_conflict * 0.20
            )

        scored = [(float(full_set_score(lst)), int(idx), list(lst)) for idx, lst in enumerate(base)]
        scored_sorted = sorted(scored, key=lambda t: (-float(t[0]), int(t[1]), tuple(t[2])))
        top_n = int(max(1, min(int(top_n or len(base)), len(base))))
        selected = [list(t[2]) for t in scored_sorted[:top_n]]
        selected_keys = {tuple(x) for x in selected}
        tail = [list(x) for x in base if tuple(x) not in selected_keys]
        out = selected + tail

        scores = [float(t[0]) for t in scored_sorted]
        before_hash = hash(str(base))
        after_hash = hash(str(out))
        ranking_changed = bool([tuple(x) for x in base] != [tuple(x) for x in out])
        selection_changed = bool([tuple(x) for x in base[:top_n]] != [tuple(x) for x in selected])

        return out, {
            "active": True,
            "applied": bool(ranking_changed or selection_changed),
            "reason": "ok" if (ranking_changed or selection_changed) else "sem_mudanca_ranking_global",
            "mode": "full_set_global_selection_pre_sanidade",
            "top_n_interno": int(top_n),
            "listas_antes_hash": before_hash,
            "listas_depois_hash": after_hash,
            "ranking_listas_mudou_pre_sanidade": bool(ranking_changed),
            "impacto_global_pre_sanidade": bool(selection_changed or before_hash != after_hash),
            "full_set_score_medio": round(float(sum(scores) / max(1, len(scores))), 10) if scores else 0.0,
            "full_set_score_max": round(float(max(scores)), 10) if scores else 0.0,
            "full_set_score_min": round(float(min(scores)), 10) if scores else 0.0,
            "scores_top": [
                {"idx_original": int(t[1]), "score": round(float(t[0]), 8), "lista": [int(x) for x in t[2]]}
                for t in scored_sorted[:min(10, len(scored_sorted))]
            ],
        }
    except Exception as e:
        return listas, {
            "active": False,
            "applied": False,
            "reason": f"full_set_global_selection_erro: {e}",
            "mode": "full_set_global_selection_pre_sanidade",
            "listas_antes_hash": hash(str(listas or [])),
            "listas_depois_hash": hash(str(listas or [])),
        }
...
def pc_v16_h8g_record_functional_role_auditor(candidate, current, score_before, score_after, role_info):
    try:
        ...
        if not isinstance(runtime_negative_samples, list):
            runtime_negative_samples = []

        runtime_roles.append(role_kind)
        runtime_negative.append(bool(is_negative))
        ...
        neutral_count = sum(1 for rk in runtime_roles if rk not in ("functional_boost", "functional_penalty"))
        negative_coexistence_count = sum(1 for flag in runtime_negative if bool(flag))
...
# ============================================================
# V16h57HO6ZOF — REAL TEMPORAL STATE (pacote -> decisão)
# Estado real calculado a partir de métricas estruturais do pacote
# e consumido dentro do gerador, não apenas exibido no auditor.
# ============================================================
def pc_v16_ho6zof_real_temporal_state(packet_metrics):
    try:
        pm = packet_metrics if isinstance(packet_metrics, dict) else {}
        overlap = float(pm.get("sobreposicao_media", 0.0) or 0.0)
        unique = int(pm.get("passageiros_unicos", 0) or 0)

        if overlap < 2.05 or unique > 18:
            state = "FECHAMENTO_FORTE"
        else:
            state = "NORMAL"

        return {
            "temporal_state": state,
            "sobreposicao_media": round(float(overlap), 4),
            "passageiros_unicos": int(unique),
            "threshold_overlap": 2.05,
            "threshold_unique": 18,
            "motivo": "ok",
        }
    except Exception as e:
...
def pc_v16_generate_lists_cooccurrence(ranking, co_matrix, n=6, k_lists=12, temporal_state="NORMAL", audit_mode=False, audit_collector=None, target_profile=None, historical_patterns_4p=None):
    """
    ...
    try:
        ...
        if audit_collector is not None:
            ...
            audit_collector.setdefault("historical_evidence_values", [])
            audit_collector.setdefault("historical_evidence_terms", [])
            audit_collector.setdefault("historical_evidence_candidates", set())
            audit_collector.setdefault("historical_evidence_samples", [])
            audit_collector.setdefault("historical_evidence_ranking_changes", 0)
            audit_collector.setdefault("diversity_guard_blocks", 0)

        historical_evidence_buffer = []

        if historical_patterns_4p is None:
            try:
                historical_patterns_4p = st.session_state.get("historical_patterns_4p", {})
            except Exception:
                historical_patterns_4p = {}
        ...
        def historical_evidence_term(candidate, current):
            try:
                ev = pc_v16_historical_closure_evidence(
                    candidate,
                    current,
                ...
                raw = float(ev.get("score_raw", 0.0) or 0.0)
                historical_evidence_buffer.append(raw)
                if len(historical_evidence_buffer) >= 6:
                    mean = float(np.mean(historical_evidence_buffer))
                    std = float(np.std(historical_evidence_buffer)) + 1e-6
                    norm = (raw - mean) / std
                else:
                    norm = raw
                term = -0.35 if not bool(ev.get("valid", False)) else float(norm) * 1.40

                if audit_collector is not None:
                    audit_collector["historical_evidence_values"].append(float(raw))
                    audit_collector["historical_evidence_terms"].append(float(term))
                    if abs(float(term)) > 1e-12:
                        audit_collector["historical_evidence_candidates"].add(int(candidate))
                    if len(audit_collector["historical_evidence_samples"]) < 24:
                        audit_collector["historical_evidence_samples"].append({
                            "candidate": int(candidate),
                            "current": [int(x) for x in list(current)],
                            "raw": round(float(raw), 8),
                            "term": round(float(term), 8),
                            "valid": bool(ev.get("valid", False)),
                            "subsets_tested": int(ev.get("subsets_tested", 0) or 0),
                            "subsets_valid": int(ev.get("subsets_valid", 0) or 0),
                            "occ_total": int(ev.get("occ_total", 0) or 0),
                            "hit4_total": int(ev.get("hit4_total", 0) or 0),
                            "samples": ev.get("samples", [])[:4],
        ...
        def diversity_guard_candidate(candidate, current):
            try:
                if len(current) < 3:
                    return True
                cand_set = set(int(x) for x in list(current) + [int(candidate)])
                if len(cand_set) < len(current) + 1:
                    if audit_collector is not None:
                        audit_collector["diversity_guard_blocks"] += 1
                    return False
                return True
            except Exception:
                return True
        ...
        def score_modulated(candidate, current):
            ...
            if temporal_state == "FECHAMENTO_FORTE":
                ...
                # HO6ZOY — CLOSURE GUIDED REAL:
                # Direcionamento explícito de fechamento dentro do score_modulated.
                # Não é flag nem pós-processo: altera o score antes do ranking/seleção.
                # Sinais reais usados:
                # - redução de dispersão normalizada
                # - densidade de pares positivos do grupo
                # - penalização de ruptura/conflito
                # - estabilidade do grupo parcial
                closure_tightness = max(0.0, 1.0 - float(spread_penalty))
                closure_density = float(group_positive_ratio)
                closure_conflict = float(group_zero_ratio)
                closure_gain = float(group_gain)
                closure_term = (
                    closure_tightness * 0.16
                    + closure_density * 0.24
                    + closure_gain * 0.18
                    - closure_conflict * 0.26
                    - conflict_ratio * 0.16
                )

                # HO6ZOY — CLOSURE LOOKAHEAD REAL:
                # avalia se, após inserir o candidato, ainda há caminhos plausíveis
                # de fechamento até n elementos. Atua dentro do score_modulated.
                remaining_slots = max(0, int(n) - len(group_after))
                lookahead_term = 0.0
                lookahead_viable_count = 0
                lookahead_quality_sum = 0.0

                if remaining_slots > 0:
                    future_pool = [int(v) for v in top if int(v) not in set(group_after)]
                    future_pool = future_pool[:max(6, min(len(future_pool), 12))]

                    future_scores = []
                    for fut in future_pool:
                        fut_pair_scores = [pair_score(fut, g) for g in group_after]
                        fut_positive_ratio = (
                            sum(1 for s in fut_pair_scores if float(s) > 0.0) / max(1, len(fut_pair_scores))
                        )
                        fut_avg_pair = (
                            sum(float(s) for s in fut_pair_scores) / max(1, len(fut_pair_scores))
                        )
                        fut_zero_ratio = (
                            sum(1 for s in fut_pair_scores if float(s) <= 0.0) / max(1, len(fut_pair_scores))
                        )
                        fut_group = list(group_after) + [fut]
                        fut_spread = max(fut_group) - min(fut_group)
                        fut_spread_penalty = min(float(fut_spread) / 60.0, 1.0)
                        fut_quality = (
                            fut_positive_ratio * 0.34
                            + fut_avg_pair * 0.10
                            + max(0.0, 1.0 - fut_spread_penalty) * 0.14
                            - fut_zero_ratio * 0.28
                        )
                        future_scores.append(float(fut_quality))

                    if future_scores:
                        future_scores_sorted = sorted(future_scores, reverse=True)
                        need = max(1, min(int(remaining_slots), len(future_scores_sorted)))
                        best_future = future_scores_sorted[:need]
                        lookahead_viable_count = sum(1 for s in best_future if float(s) > 0.0)
                        lookahead_quality_sum = sum(float(s) for s in best_future) / max(1, len(best_future))
                        lookahead_viability = float(lookahead_viable_count) / max(1, need)
                        lookahead_term = (
                            lookahead_quality_sum * 0.22
                            + lookahead_viability * 0.18
                            - (1.0 - lookahead_viability) * 0.20
                        )

                # HO6ZOY — TARGET SHAPE CONVERGENCE:
                # avalia se o grupo parcial já possui forma estrutural saudável
                # de fechamento, além de qualidade atual e viabilidade futura.
                target_density = float(group_positive_ratio)
                target_conflict = float(group_zero_ratio)
                target_spread_health = max(0.0, 1.0 - abs(float(spread_penalty) - 0.42))
                target_balance = max(0.0, 1.0 - abs(float(group_positive_ratio) - 0.72))
                target_gain_health = max(0.0, min(1.0, float(group_gain) + 0.50))

                target_shape_term = (
                    target_density * 0.20
                    + target_balance * 0.18
                    + target_spread_health * 0.14
                    + target_gain_health * 0.12
                    - target_conflict * 0.24
                )

                # HO6ZOY — FINAL SET CLOSURE REAL:
                # Simula uma conclusão leve do conjunto após inserir o candidato
                # e avalia a forma do conjunto completo estimado dentro do candidate_fit.
                final_set_closure_term = 0.0
                completed_group = list(group_after)

                if len(completed_group) < int(n):
                    future_pool2 = [int(v) for v in top if int(v) not in set(completed_group)]
                    future_pool2 = future_pool2[:max(8, min(len(future_pool2), 16))]

                    while len(completed_group) < int(n) and future_pool2:
                        best_fut = None
                        best_fut_score = -1e18
                        for fut in future_pool2:
                            fut_pair_scores2 = [pair_score(fut, g) for g in completed_group]
                            fut_positive_ratio2 = (
                                sum(1 for s in fut_pair_scores2 if float(s) > 0.0) / max(1, len(fut_pair_scores2))
                            )
                            fut_avg_pair2 = (
                                sum(float(s) for s in fut_pair_scores2) / max(1, len(fut_pair_scores2))
                            )
                            fut_zero_ratio2 = (
                                sum(1 for s in fut_pair_scores2 if float(s) <= 0.0) / max(1, len(fut_pair_scores2))
                            )
                            fut_group2 = list(completed_group) + [fut]
                            fut_spread2 = max(fut_group2) - min(fut_group2)
                            fut_spread_penalty2 = min(float(fut_spread2) / 60.0, 1.0)
                            fut_score2 = (
                                fut_positive_ratio2 * 0.30
                                + fut_avg_pair2 * 0.12
                                + max(0.0, 1.0 - fut_spread_penalty2) * 0.12
                                - fut_zero_ratio2 * 0.26
                            )
                            if fut_score2 > best_fut_score:
                                best_fut_score = float(fut_score2)
                                best_fut = int(fut)

                        if best_fut is None:
                            break
                        completed_group.append(int(best_fut))
                        future_pool2 = [v for v in future_pool2 if int(v) != int(best_fut)]

                if len(completed_group) == int(n):
                    completed_pair_scores = []
                    for _i in range(len(completed_group)):
                        for _j in range(_i + 1, len(completed_group)):
                            completed_pair_scores.append(pair_score(completed_group[_i], completed_group[_j]))

                    completed_pair_avg = sum(float(s) for s in completed_pair_scores) / max(1, len(completed_pair_scores))
                    completed_positive_ratio = (
                        sum(1 for s in completed_pair_scores if float(s) > 0.0) / max(1, len(completed_pair_scores))
                    )
                    completed_zero_ratio = (
                        sum(1 for s in completed_pair_scores if float(s) <= 0.0) / max(1, len(completed_pair_scores))
                    )
                    completed_spread = max(completed_group) - min(completed_group)
                    completed_spread_penalty = min(float(completed_spread) / 60.0, 1.0)

                    completed_shape_balance = max(0.0, 1.0 - abs(float(completed_positive_ratio) - 0.72))
                    completed_spread_health = max(0.0, 1.0 - abs(float(completed_spread_penalty) - 0.42))

                    final_set_closure_term = (
                        completed_positive_ratio * 0.22
                        + completed_pair_avg * 0.10
                        + completed_shape_balance * 0.18
                        + completed_spread_health * 0.12
                        - completed_zero_ratio * 0.26
                    )

                # HO6ZOY — REAL TARGET PROFILE CLOSURE:
                # compara o conjunto completo estimado com perfil histórico real,
                # reduzindo erro de forma entre o grupo e as séries reais.
                real_target_profile_term = 0.0
                if isinstance(target_profile, dict) and bool(target_profile.get("ok")) and len(completed_group) == int(n):
                    _cg = sorted(int(x) for x in completed_group[:int(n)])
                    _spread_real = max(_cg) - min(_cg)
                    _low_real = sum(1 for x in _cg if int(x) <= 17)
                    _mid_real = sum(1 for x in _cg if 18 <= int(x) <= 34)
                    _high_real = sum(1 for x in _cg if int(x) >= 35)
                    _even_real = sum(1 for x in _cg if int(x) % 2 == 0)
                    _consec_real = 0
                    for _i in range(len(_cg) - 1):
                        if int(_cg[_i + 1]) - int(_cg[_i]) == 1:
                            _consec_real += 1
                    _shape_error = (
                        abs(float(_spread_real) - float(target_profile.get("spread_mean", 0.0))) / 50.0
                        + abs(float(_low_real) - float(target_profile.get("low_mean", 0.0))) / max(1.0, float(n))
                        + abs(float(_mid_real) - float(target_profile.get("mid_mean", 0.0))) / max(1.0, float(n))
                        + abs(float(_high_real) - float(target_profile.get("high_mean", 0.0))) / max(1.0, float(n))
                        + abs(float(_even_real) - float(target_profile.get("even_mean", 0.0))) / max(1.0, float(n))
                        + abs(float(_consec_real) - float(target_profile.get("consecutive_mean", 0.0))) / max(1.0, float(n))
                    )
                    _shape_fit = max(0.0, 1.0 - float(_shape_error) / 6.0)
                    real_target_profile_term = (
                        _shape_fit * 0.34
                        + max(0.0, 1.0 - abs(float(completed_positive_ratio) - 0.72)) * 0.10
                        - float(completed_zero_ratio) * 0.18
                    )

                # HO6ZOY — CLOSURE VIABILITY DECISION:
                # múltiplos caminhos de fechamento, com diversidade controlada,
                # avaliando o conjunto completo possível antes da decisão.
                closure_viability_term = 0.0
                closure_viability_paths = []

                if len(group_after) < int(n):
                    base_future_pool = [int(v) for v in top if int(v) not in set(group_after)]
                    base_future_pool = base_future_pool[:max(10, min(len(base_future_pool), 18))]

                    def _completed_group_score(_vals):
                        _vals = sorted(dict.fromkeys(int(x) for x in _vals[:int(n)]))
                        if len(_vals) != int(n):
                            return -1.0
                        ...
                        for _i in range(len(_vals)):
                            for _j in range(_i + 1, len(_vals)):
                                _pair_scores.append(pair_score(_vals[_i], _vals[_j]))

                        _pair_avg = sum(float(s) for s in _pair_scores) / max(1, len(_pair_scores))
                        _positive_ratio = sum(1 for s in _pair_scores if float(s) > 0.0) / max(1, len(_pair_scores))
                        _zero_ratio = sum(1 for s in _pair_scores if float(s) <= 0.0) / max(1, len(_pair_scores))
                        _spread = max(_vals) - min(_vals)
                        _spread_penalty = min(float(_spread) / 60.0, 1.0)

                        _historical_fit = 0.0
                        if isinstance(target_profile, dict) and bool(target_profile.get("ok")):
                            _low = sum(1 for x in _vals if int(x) <= 17)
                            _mid = sum(1 for x in _vals if 18 <= int(x) <= 34)
                            _high = sum(1 for x in _vals if int(x) >= 35)
                            _even = sum(1 for x in _vals if int(x) % 2 == 0)
                            _consec = 0
                            for _k in range(len(_vals) - 1):
                                if int(_vals[_k + 1]) - int(_vals[_k]) == 1:
                                    _consec += 1
                            _hist_error = (
                                abs(float(_spread) - float(target_profile.get("spread_mean", 0.0))) / 50.0
                                + abs(float(_low) - float(target_profile.get("low_mean", 0.0))) / max(1.0, float(n))
                                + abs(float(_mid) - float(target_profile.get("mid_mean", 0.0))) / max(1.0, float(n))
                                + abs(float(_high) - float(target_profile.get("high_mean", 0.0))) / max(1.0, float(n))
                                + abs(float(_even) - float(target_profile.get("even_mean", 0.0))) / max(1.0, float(n))
                                + abs(float(_consec) - float(target_profile.get("consecutive_mean", 0.0))) / max(1.0, float(n))
                            )
                            _historical_fit = max(0.0, 1.0 - float(_hist_error) / 6.0)

                        return float(
                            _positive_ratio * 0.26
                            + _pair_avg * 0.10
                            + max(0.0, 1.0 - abs(float(_positive_ratio) - 0.72)) * 0.12
                            + max(0.0, 1.0 - abs(float(_spread_penalty) - 0.42)) * 0.10
                            + _historical_fit * 0.18
                            - _zero_ratio * 0.26
                        )

                    for _path_idx in range(8):
                        _pool = list(base_future_pool)
                        if not _pool:
                            break
                        _shift = int(_path_idx) % max(1, len(_pool))
                        _pool = _pool[_shift:] + _pool[:_shift]
                        _sim_group = list(group_after)

                        while len(_sim_group) < int(n) and _pool:
                            _candidates_path = _pool[:max(4, min(len(_pool), 8))]
                            _best_val = None
                            _best_score = -1e18

                            for _cand2 in _candidates_path:
                                _trial = list(_sim_group) + [int(_cand2)]
                                _trial_pair_scores = [pair_score(int(_cand2), int(g)) for g in _sim_group]
                                _trial_positive = sum(1 for s in _trial_pair_scores if float(s) > 0.0) / max(1, len(_trial_pair_scores))
                                _trial_avg = sum(float(s) for s in _trial_pair_scores) / max(1, len(_trial_pair_scores))
                                _trial_zero = sum(1 for s in _trial_pair_scores if float(s) <= 0.0) / max(1, len(_trial_pair_scores))
                                _trial_spread = max(_trial) - min(_trial)
                                _trial_spread_penalty = min(float(_trial_spread) / 60.0, 1.0)

                                if _path_idx % 3 == 0:
                                    _path_score = _trial_positive * 0.34 + _trial_avg * 0.10 - _trial_zero * 0.24 - _trial_spread_penalty * 0.08
                                elif _path_idx % 3 == 1:
                                    _path_score = max(0.0, 1.0 - abs(float(_trial_spread_penalty) - 0.42)) * 0.24 + _trial_positive * 0.22 + rank_strength(_cand2) * 0.05 - _trial_zero * 0.22
                                else:
                                    _hist_hint = 0.0
                                    if isinstance(target_profile, dict) and bool(target_profile.get("ok")):
                                        _trial_low = sum(1 for x in _trial if int(x) <= 17)
                                        _trial_mid = sum(1 for x in _trial if 18 <= int(x) <= 34)
                                        _trial_high = sum(1 for x in _trial if int(x) >= 35)
                                        _hist_hint = (
                                            max(0.0, 1.0 - abs(float(_trial_low) - float(target_profile.get("low_mean", 0.0))) / max(1.0, float(n))) * 0.10
                                            + max(0.0, 1.0 - abs(float(_trial_mid) - float(target_profile.get("mid_mean", 0.0))) / max(1.0, float(n))) * 0.10
                                            + max(0.0, 1.0 - abs(float(_trial_high) - float(target_profile.get("high_mean", 0.0))) / max(1.0, float(n))) * 0.10
                                        )
                                    _path_score = _trial_positive * 0.18 + _trial_avg * 0.08 + _hist_hint + max(0.0, 1.0 - _trial_spread_penalty) * 0.10 - _trial_zero * 0.20

                                _path_score += ((_path_idx % 5) - 2) * 0.003

                                if float(_path_score) > float(_best_score):
                                    _best_score = float(_path_score)
                                    _best_val = int(_cand2)

                            if _best_val is None:
                                break
                            _sim_group.append(int(_best_val))
                            _pool = [int(v) for v in _pool if int(v) != int(_best_val)]

                        if len(_sim_group) == int(n):
                            closure_viability_paths.append(_completed_group_score(_sim_group))

                    if closure_viability_paths:
                        _good_paths = [float(s) for s in closure_viability_paths if float(s) > 0.0]
                        _avg_path = sum(float(s) for s in closure_viability_paths) / max(1, len(closure_viability_paths))
                        _good_ratio = len(_good_paths) / max(1, len(closure_viability_paths))
                        _best_path = max(float(s) for s in closure_viability_paths)
                ...
                return float(
                    pair_sum * 0.018
                    + pair_avg * 0.16
                    + rank_strength(candidate) * 0.0015
                    + group_coherence_bonus
                    + closure_term
                    + lookahead_term
                    + target_shape_term
        ...
        def list_score(vals):
            ...
            if temporal_state == "FECHAMENTO_FORTE":
                # HO6ZOY — ordenação final das listas por coerência global:
                ...
                closure_list_term = (
                    max(0.0, 1.0 - float(spread_penalty)) * 0.14
                    + float(positive_ratio) * 0.22
                    - float(zero_ratio) * 0.24
                )

                # HO6ZOY — list_score com viabilidade interna da lista como unidade.
                lookahead_list_term = (
                    float(positive_ratio) * 0.18
                    + float(avg_positive_pair) * 0.08
                    + max(0.0, 1.0 - float(spread_penalty)) * 0.10
                    - float(zero_ratio) * 0.20
                )

                # HO6ZOY — list_score por formato estrutural final.
                target_shape_list_term = (
                    float(positive_ratio) * 0.18
                    + max(0.0, 1.0 - abs(float(positive_ratio) - 0.72)) * 0.16
                    + max(0.0, 1.0 - abs(float(spread_penalty) - 0.42)) * 0.12
                    - float(zero_ratio) * 0.22
                )

                # HO6ZOY — list_score também avalia o conjunto final como unidade.
                final_set_list_term = (
                    float(positive_ratio) * 0.20
                    + float(avg_positive_pair) * 0.10
                    + max(0.0, 1.0 - abs(float(positive_ratio) - 0.72)) * 0.16
                    + max(0.0, 1.0 - abs(float(spread_penalty) - 0.42)) * 0.10
                    - float(zero_ratio) * 0.24
                )
        ...
        if audit_collector is not None:
            ...
            try:
                hev = audit_collector.get("historical_evidence_values", []) or []
                het = audit_collector.get("historical_evidence_terms", []) or []
                audit_collector["historical_closure_evidence_active"] = True
                audit_collector["historical_evidence_score_medio"] = round(float(sum(hev) / max(1, len(hev))), 10) if hev else 0.0
                audit_collector["historical_evidence_score_max"] = round(float(max(hev)), 10) if hev else 0.0
                audit_collector["historical_evidence_score_min"] = round(float(min(hev)), 10) if hev else 0.0
                audit_collector["historical_evidence_term_medio"] = round(float(sum(het) / max(1, len(het))), 10) if het else 0.0
                audit_collector["historical_evidence_term_max"] = round(float(max(het)), 10) if het else 0.0
                audit_collector["historical_evidence_term_min"] = round(float(min(het)), 10) if het else 0.0
                audit_collector["historical_evidence_candidates_affected"] = int(len(audit_collector.get("historical_evidence_candidates", set())))
                audit_collector["historical_evidence_ranking_changes"] = int(audit_collector.get("historical_evidence_ranking_changes", 0))
                audit_collector["diversity_guard_blocks"] = int(audit_collector.get("diversity_guard_blocks", 0))
                try:
                    audit_collector["historical_evidence_candidates"] = sorted([int(x) for x in audit_collector.get("historical_evidence_candidates", set())])[:30]
                except Exception:
                    audit_collector["historical_evidence_candidates"] = []
            except Exception:
                audit_collector["historical_closure_evidence_active"] = False
...
def pc_v16_new_packet_generator(listas_totais, *, ranking_vals=None, historico_df=None, n_alvo=6, seed=0, max_lists=None):
    try:
        ...
        try:
            st.session_state["v16h57HO6ZOY_generator_call_count"] = int(st.session_state.get("v16h57HO6ZOY_generator_call_count", 0)) + 1
            _steps = st.session_state.get("v16h57HO6ZOY_generator_call_steps")
            if not isinstance(_steps, list):
                _steps = []
            _steps.append({"count": int(st.session_state.get("v16h57HO6ZOY_generator_call_count", 1)), "arg_n": int(len(listas_totais or []))})
            st.session_state["v16h57HO6ZOY_generator_call_steps"] = _steps
        except Exception:
            pass
        base = []
        for lst in (listas_totais or []):
        ...
        # HO6ZOF REAL — estado calculado a partir de métricas reais do pacote base
        ho6zof_state_metrics = pc_packet_audit_dict(base, "HO6ZOF_STATE_BASE")
        ho6zof_state_info = pc_v16_ho6zof_real_temporal_state(ho6zof_state_metrics)
        temporal_state = str(ho6zof_state_info.get("temporal_state", "n/d"))
        try:
            st.session_state["v16h57HO6ZOF_REAL_temporal_state_info"] = dict(ho6zof_state_info)
        except Exception:
            pass
        ...
        # HO6ZOY — perfil histórico real de fechamento das séries.
        target_profile = {"ok": False, "motivo": "sem_series_hist"}
        try:
            _profiles = []
            for _s in (series_hist or [])[-600:]:
                _vals = sorted(dict.fromkeys(int(x) for x in list(_s)[:int(n_alvo)]))
                if len(_vals) != int(n_alvo):
                    continue
                _spread = max(_vals) - min(_vals)
                _low = sum(1 for x in _vals if int(x) <= 17)
                _mid = sum(1 for x in _vals if 18 <= int(x) <= 34)
                ...
                for _i in range(len(_vals) - 1):
                    if int(_vals[_i + 1]) - int(_vals[_i]) == 1:
                        _consec += 1
                _profiles.append({
                    ...
                    "high": float(_high),
                    "even": float(_even),
                    "consecutive": float(_consec),
                })
            if _profiles:
                target_profile = {
                    "ok": True,
                    "spread_mean": float(sum(p["spread"] for p in _profiles) / len(_profiles)),
                    "low_mean": float(sum(p["low"] for p in _profiles) / len(_profiles)),
                    "mid_mean": float(sum(p["mid"] for p in _profiles) / len(_profiles)),
                    "high_mean": float(sum(p["high"] for p in _profiles) / len(_profiles)),
                    "even_mean": float(sum(p["even"] for p in _profiles) / len(_profiles)),
                    "consecutive_mean": float(sum(p["consecutive"] for p in _profiles) / len(_profiles)),
                    "amostras": int(len(_profiles)),
                    "motivo": "ok",
                }
        except Exception as _tp_e:
            target_profile = {"ok": False, "motivo": f"erro_target_profile: {_tp_e}"}
        ...
        k_gen = min(max_lists, max(9, min(12, max_lists)))

        historical_patterns_4p = pc_v16_build_historical_patterns_4p(series_hist, n_alvo=int(n_alvo))
        try:
            st.session_state["historical_patterns_4p"] = historical_patterns_4p
        except Exception:
            pass
        ...
        # HO6ZOY — FULL SET GLOBAL SELECTION LAYER (pré-sanidade)
        full_set_global_audit = {
            "active": False,
            "applied": False,
        ...
        try:
            out, full_set_global_audit = pc_v16_full_set_global_selection_layer(
                out,
                co_matrix=co,
                target_profile=target_profile,
                n_alvo=int(n_alvo),
                top_n=min(len(out), max(10, int(k_gen))),
            )
        except Exception as _fsg_e:
            full_set_global_audit = {
                "active": False,
                "applied": False,
                "reason": f"full_set_global_selection_erro: {_fsg_e}",
                "mode": "full_set_global_selection_pre_sanidade",
            }
        ...
        return out, {
            ...
            "closure_lookahead_mode": "candidate_fit_future_completion_viability",
            "target_shape_convergence_active": True,
            "target_shape_mode": "candidate_fit_structural_form_density_balance_spread",
            "final_set_closure_real_active": True,
            "final_set_closure_mode": "candidate_fit_completed_group_estimate_before_selection",
            "real_target_profile_closure_active": True,
            "real_target_profile_mode": "candidate_fit_completed_group_vs_historical_series_profile",
            "real_target_profile_info": target_profile,
            "closure_viability_decision_active": True,
            "closure_viability_mode": "candidate_fit_diverse_completed_group_paths",
            "closure_viability_paths_per_candidate": 8,
...
# ============================================================
def v16h57FS_clear_mode6_packet_state():
    removed = []
    keys = [
        "modo6_listas",
        "modo6_listas_top10",
        ...
        "v16h57HO6ZOY_auditor",
        "v16h57HO6ZOY_generator_call_count",
        "v16h57HO6ZOY_generator_call_steps",
        "v16h57HO6ZOY_pre_sanidade_top10",
        "v16h57HO6ZOY_post_sanidade_top10",
    ]
    try:
        for k in keys:
...
def pc_v16_build_auditor_ho6w(*, npgen_info=None, pre_sanidade_top10=None, post_sanidade_top10=None):
    try:
        ...
        gen_calls = int(st.session_state.get("v16h57HO6ZOY_generator_call_count", 0) or 0)
        ...
        chegou_top10 = bool(fm_applied or changed_indices or promoted_from_tail or top10_changed)

        auditor = {
            "status": "OK",
            "temporal_state": temporal_state,
            "temporal_state_info": temporal_state_info,
            ...
            "overlap_after": float(fm.get("overlap_after", 0.0) or 0.0),
            "score_base_packet": float(fm.get("score_base_packet", 0.0) or 0.0),
            "score_final_packet": float(fm.get("score_final_packet", 0.0) or 0.0),
        }
        ...
        st.session_state["v16h57HO6ZOY_auditor"] = auditor
        return auditor
    except Exception as e:
        auditor = {
            "status": "INVALIDO",
            ...
            "unicidade": "FALHA",
            "generator_call_count": int(st.session_state.get("v16h57HO6ZOY_generator_call_count", 0) or 0),
            "ponto_fluxo": "FALHA",
            "antes_sanidade": "OK",
            "mudou_pacote": "NAO",
            "chegou_top10": "NAO",
...
def pc_modo6_gerar_pacote_top10_silent(df: pd.DataFrame, calib_override=None) -> Tuple[List[List[int]], Dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            try:
                if "listas_filtradas" in locals() and listas_filtradas:
                    listas_base_inter = listas_filtradas
                elif "_listas_pre_sanidade" in locals() and _listas_pre_sanidade:
                    listas_base_inter = _listas_pre_sanidade
            except Exception:
                listas_base_inter = []
...
if painel == "🎯 Compressão do Alvo (Observacional)":
    ...
    # V16h57CN — SESSION STATE CONTROL (FORCE FRESH PACKET)
    # ------------------------------------------------------------
    _removed_fresh_keys = v16h57FS_clear_mode6_packet_state()
...
def v9_classificar_memoria_borda(*, df_res: Optional[pd.DataFrame], total_hits: int, pct: Dict[str, float]) -> Dict[str, Any]:
    ...
    try:
        ...
        if fora_pct >= 65.0:
            return {"status": "OK",
            "temporal_state": "FECHAMENTO_FORTE",
            "impacto_temporal_real": True, "motivo_curto": "Lastro existe, mas fora_do_pacote está alto.", "n_alvos_avaliados": int(n_alvos)}

        return {"status": "OK",
            "temporal_state": "FECHAMENTO_FORTE",
            "impacto_temporal_real": True, "motivo_curto": "Lastro adequado para baliza informativa.", "n_alvos_avaliados": int(n_alvos)}
    except Exception:
...
if painel == "🎯 Modo 6 Acertos — Execução":
    ...
    st.session_state["v16h57FS_exec_trace"] = []
...
# ------------------------------------------------------------
    ...
    try:
        st.session_state["v16h57HO6ZOY_pre_sanidade_top10"] = [list(lst) for lst in (listas_brutas or [])[:10]]
    except Exception:
        st.session_state["v16h57HO6ZOY_pre_sanidade_top10"] = []
    try:
    ...
    try:
        st.session_state["v16h57HO6ZOY_post_sanidade_top10"] = [list(lst) for lst in (listas_top10 or [])[:10]]
    except Exception:
        st.session_state["v16h57HO6ZOY_post_sanidade_top10"] = []

    try:
        _auditor_ho6 = pc_v16_build_auditor_ho6w(
            npgen_info=(_npgen_dx_info if isinstance(_npgen_dx_info, dict) else {}),
            pre_sanidade_top10=st.session_state.get("v16h57HO6ZOY_pre_sanidade_top10") or [],
            post_sanidade_top10=st.session_state.get("v16h57HO6ZOY_post_sanidade_top10") or [],
        )
    except Exception as _aud_e:
        _auditor_ho6 = {"status": "INVALIDO", "motivo": f"auditor_erro: {_aud_e}"}

    st.markdown("### 🔎 AUDITOR HO6ZOY")
...
try:
    ...
    if listas_ref:
        ...
        _trace_exec = st.session_state.get("v16h57FS_exec_trace", [])
        ...
        _src_trace = st.session_state.get("v16h57FS_source_detector", [])
